...
export function ContextMenuItem(props: ContextMenuItemProps) {
  ...
  const renderedNestedItems = useMemo(
  ...
  return (
    <>
      <button
        ...
        type="button"
        className={cn(
          "flex w-full items-center gap-2 rounded-sm p-1.5 text-left text-11 text-secondary select-none",
          {
      ...
      {hasNestedItems && isNestedOpen && (
        <Portal container={contextMenuContext?.portalContainer}>
          <div
            ...
            style={styles.popper}
            {...attributes.popper}
            className="fixed z-[35] min-w-[12rem] overflow-hidden rounded-md border-[0.5px] border-subtle-1 bg-surface-1 p-2.5 text-11"
