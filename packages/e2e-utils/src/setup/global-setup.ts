...
export default async function globalSetup(): Promise<void> {
  ...
  try {
    ...
    // 3. Intentar sign-in; si falla, sign-up
    ...
    // 4. Persistir storage state (cookies de sesión)
    await ctx.storageState({ path: STORAGE_PATH });

    // 5. Crear workspace con slug único
    ...
    // 9. Persistir context de seed para teardown
    const seedMeta = {
      slug: workspace.slug,
      email: EMAIL,
    ...
    fs.writeFileSync(
      path.resolve(path.dirname(STORAGE_PATH), "e2e-seed.json"),
