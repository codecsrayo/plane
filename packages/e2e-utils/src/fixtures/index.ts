...
async function del(request: import("@playwright/test").APIRequestContext, path: string, csrf: string) {
  await request.delete(`${apiBase()}${path}`, { headers: { "X-CSRFToken": csrf } });
}
...
export const test = base.extend<PlaneFixtures>({
  ...
  freshIssue: async ({ request, csrf }, use) => {
    const res = await request.post(`${apiBase()}/api/workspaces/${slug()}/projects/${pid()}/issues`, {
  ...
  freshCycle: async ({ request, csrf }, use) => {
    ...
    const res = await request.post(`${apiBase()}/api/workspaces/${slug()}/projects/${pid()}/cycles`, {
      headers: { "X-CSRFToken": csrf },
      data: {
  ...
  freshLabel: async ({ request, csrf }, use) => {
    const res = await request.post(`${apiBase()}/api/workspaces/${slug()}/projects/${pid()}/labels`, {
      headers: { "X-CSRFToken": csrf },
  ...
  freshModule: async ({ request, csrf }, use) => {
    const res = await request.post(`${apiBase()}/api/workspaces/${slug()}/projects/${pid()}/modules`, {
      headers: { "X-CSRFToken": csrf },
  ...
  freshPage: async ({ request, csrf }, use) => {
    const res = await request.post(`${apiBase()}/api/workspaces/${slug()}/projects/${pid()}/pages`, {
      headers: { "X-CSRFToken": csrf },
  ...
  freshView: async ({ request, csrf }, use) => {
    const res = await request.post(`${apiBase()}/api/workspaces/${slug()}/views`, {
      headers: { "X-CSRFToken": csrf },
  ...
  freshWebhook: async ({ request, csrf }, use) => {
    ...
    const res = await request.post(`${apiBase()}/api/workspaces/${slug()}/webhooks`, {
      headers: { "X-CSRFToken": csrf },
  ...
  freshApiToken: async ({ request, csrf }, use) => {
    const res = await request.post(`${apiBase()}/api/api-tokens`, {
      headers: { "X-CSRFToken": csrf },
