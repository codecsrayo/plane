...
export abstract class BaseIssuesStore implements IBaseIssuesStore {
  ...
  updateGroupedIssueIds(
    ...
    for (const groupedIssueId in groupedIssues) {
      const issueGroup = groupedIssues[groupedIssueId];
      const issueGroupCount = groupedIssueCount[groupedIssueId];

      // update the groupId's issue count
      set(this.groupedIssueCount, [groupedIssueId], issueGroupCount);

      // This updates the group issue list in the store, if the issueGroup is a string
      const storeUpdated = this.updateIssueGroup(issueGroup, [groupedIssueId]);
      ...
      for (const groupedSubGroupId in issueGroup) {
        const issueSubGroup = (issueGroup as TGroupedIssues)[groupedSubGroupId];
        const issueSubGroupCount = groupedIssueCount[getGroupKey(groupedIssueId, groupedSubGroupId)];

        // update the subGroupId's issue count
        set(this.groupedIssueCount, [getGroupKey(groupedIssueId, groupedSubGroupId)], issueSubGroupCount);
        // This updates the subgroup issue list in the store
        this.updateIssueGroup(issueSubGroup, [groupedIssueId, groupedSubGroupId]);
      }
    }
  }
