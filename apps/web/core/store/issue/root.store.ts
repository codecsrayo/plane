...
export class IssueRootStore implements IIssueRootStore {
  ...
  constructor(rootStore: RootStore, serviceType: TIssueServiceType = EIssueServiceType.ISSUES) {
    ...
    // MobX strict-mode (enableStaticRendering/enforceActions): cualquier
    // mutacion de observables debe ocurrir dentro de una action. Un autorun
    // que sincroniza 15+ observables a partir del router dispara:
    ...
    //    IssueRootStore@N.workspaceSlug"
    // Envolvemos el cuerpo del autorun en runInAction() para que todas las
    // mutaciones ocurran en un unico batch transaccional.
    autorun(() => {
      runInAction(() => {
