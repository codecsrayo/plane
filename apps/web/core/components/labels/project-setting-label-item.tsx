...
export function ProjectSettingLabelItem(props: Props) {
  ...
  const removeFromGroup = (issueLabel: IIssueLabel) => {
    if (!workspaceSlug || !projectId) return;

    updateLabel(workspaceSlug.toString(), projectId.toString(), issueLabel.id, {
      parent: null,
    });
  };
  ...
  return (
    <LabelDndHOC label={label} isGroup={false} isChild={isChild} isLastChild={isLastChild} onDrop={onDrop}>
      {(isDragging, isDroppingInLabel, dragHandleRef) => (
        ...
        >
          <div
            className={`group relative flex items-center justify-between gap-2 gap-y-3 rounded-sm px-1 py-3 ${
