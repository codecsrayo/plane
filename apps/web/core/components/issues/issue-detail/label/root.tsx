...
export const IssueLabel = observer(function IssueLabel(props: TIssueLabel) {
  ...
  const labelOperations: TLabelOperations = useMemo(
    () => ({
      updateIssue: async (
        ...
        try {
          ...
          else await updateIssue(targetWorkspaceSlug, targetProjectId, targetIssueId, data);
        } catch (_error) {
          setToast({
            title: t("toast.error"),
      ...
      createLabel: async (targetWorkspaceSlug: string, targetProjectId: string, data: Partial<IIssueLabel>) => {
        try {
          const labelResponse = await createLabel(targetWorkspaceSlug, targetProjectId, data);
          if (!isInboxIssue)
            setToast({
          ...
          if (
            ...
            typeof error === "object" &&
            "error" in error &&
            (error as { error: unknown }).error === "Label with the same name already exists in the project"
          )
    ...
    [updateIssue, createLabel, isInboxIssue, onLabelUpdate, t]
  );
