...
export const getWeeksBetweenTwoDates = (
  ...
  while (currentTimestamp <= endTimestamp) {
    const weekStartDate = new Date(currentTimestamp);
    const weekEndDate = new Date(currentTimestamp + 6 * 24 * 60 * 60 * 1000);
    ...
    currentTimestamp = weekStartDate.getTime() + 7 * 24 * 60 * 60 * 1000;
  }
