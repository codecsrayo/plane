 ...
 *   /auth/slack/callback?code=XXXXX&state={workspaceSlug}
 *
 * This page:
...
export default function SlackIntegrationCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!code || !workspaceSlug) {
      ...
      window.opener?.postMessage({ type: "slack-integration", success: false }, window.location.origin);
      return;
    }
    ...
    appInstallationService
      ...
      .then((result) => {
        ...
        window.opener?.postMessage({ type: "slack-integration", success: true }, window.location.origin);
      ...
      .catch((err) => {
        const msg =
          err?.data?.error ?? err?.error ?? err?.statusText ?? "Failed to complete Slack integration. Please try again.";
        setErrorMessage(msg);
        setStatus("error");
        window.opener?.postMessage({ type: "slack-integration", success: false, error: msg }, window.location.origin);
      });
  }, [searchParams]);
      ...
      <div className="shadow-sm flex flex-col items-center gap-4 rounded-lg border border-subtle bg-surface-2 p-10">
        {status === "processing" && (
          <>
            ...
            <p className="text-body-sm-medium text-secondary">Completing Slack integration…</p>
          </>
        )}
        ...
        {status === "success" && (
          <>
            ...
            <p className="text-body-sm-medium">Slack integrated successfully! Closing…</p>
          </>
        )}
