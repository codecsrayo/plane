/**
 * Copyright (c) 2023-present Plane Software, Inc. and contributors
 * SPDX-License-Identifier: AGPL-3.0-only
 * See the LICENSE file for details.
 */

import { LayoutGrid } from "lucide-react";
// plane imports
import { CycleIcon, ModuleIcon, PageIcon, ProjectIcon, ViewsIcon } from "@plane/propel/icons";
import type {
  IWorkspaceDefaultSearchResult,
  IWorkspaceIssueSearchResult,
  IWorkspacePageSearchResult,
  IWorkspaceProjectSearchResult,
  IWorkspaceSearchResult,
} from "@plane/types";
import { generateWorkItemLink } from "@plane/utils";
// plane web components
import { IssueIdentifier } from "@/plane-web/components/issues/issue-details/issue-identifier";

/**
 * Per-group contract. The callbacks previously accepted `any`, which defeated
 * the purpose of the per-entry search-result interfaces (IWorkspaceIssueSearchResult,
 * IWorkspacePageSearchResult, …). The generic `T` pins each entry to its
 * specific result shape so TypeScript validates `itemName`/`path` against
 * the real field set at definition time.
 */
export type TCommandGroup<T> = {
  icon: React.ReactNode | null;
  itemName: (item: T) => React.ReactNode;
  path: (item: T, projectId: string | undefined) => string;
  title: string;
};

export type TCommandGroups = {
  cycle: TCommandGroup<IWorkspaceDefaultSearchResult>;
  issue: TCommandGroup<IWorkspaceIssueSearchResult>;
  issue_view: TCommandGroup<IWorkspaceDefaultSearchResult>;
  module: TCommandGroup<IWorkspaceDefaultSearchResult>;
  page: TCommandGroup<IWorkspacePageSearchResult>;
  project: TCommandGroup<IWorkspaceProjectSearchResult>;
  workspace: TCommandGroup<IWorkspaceSearchResult>;
};

export const commandGroups: TCommandGroups = {
  cycle: {
    icon: <CycleIcon className="size-3" />,
    itemName: (cycle) => (
      <h6>
        <span className="text-11 text-tertiary">{cycle.project__identifier}</span> {cycle.name}
      </h6>
    ),
    path: (cycle) => `/${cycle?.workspace__slug}/projects/${cycle?.project_id}/cycles/${cycle?.id}`,
    title: "Cycles",
  },
  issue: {
    icon: null,
    itemName: (issue) => (
      <div className="flex gap-2">
        <IssueIdentifier
          projectId={issue.project_id}
          issueTypeId={issue.type_id}
          projectIdentifier={issue.project__identifier}
          issueSequenceId={issue.sequence_id}
          size="xs"
        />{" "}
        {issue.name}
      </div>
    ),
    path: (issue) =>
      generateWorkItemLink({
        workspaceSlug: issue?.workspace__slug,
        projectId: issue?.project_id,
        issueId: issue?.id,
        projectIdentifier: issue.project__identifier,
        sequenceId: issue?.sequence_id,
      }),
    title: "Work items",
  },
  issue_view: {
    icon: <ViewsIcon className="size-3" />,
    itemName: (view) => (
      <h6>
        <span className="text-11 text-tertiary">{view.project__identifier}</span> {view.name}
      </h6>
    ),
    path: (view) => `/${view?.workspace__slug}/projects/${view?.project_id}/views/${view?.id}`,
    title: "Views",
  },
  module: {
    icon: <ModuleIcon className="size-3" />,
    itemName: (module) => (
      <h6>
        <span className="text-11 text-tertiary">{module.project__identifier}</span> {module.name}
      </h6>
    ),
    path: (module) => `/${module?.workspace__slug}/projects/${module?.project_id}/modules/${module?.id}`,
    title: "Modules",
  },
  page: {
    icon: <PageIcon className="size-3" />,
    itemName: (page) => (
      <h6>
        <span className="text-11 text-tertiary">{page.project__identifiers?.[0]}</span> {page.name}
      </h6>
    ),
    path: (page, projectId) => {
      let redirectProjectId = page?.project_ids?.[0];
      if (!!projectId && page?.project_ids?.includes(projectId)) redirectProjectId = projectId;
      return redirectProjectId
        ? `/${page?.workspace__slug}/projects/${redirectProjectId}/pages/${page?.id}`
        : `/${page?.workspace__slug}/wiki/${page?.id}`;
    },
    title: "Pages",
  },
  project: {
    icon: <ProjectIcon className="size-3" />,
    itemName: (project) => project?.name,
    path: (project) => `/${project?.workspace__slug}/projects/${project?.id}/issues/`,
    title: "Projects",
  },
  workspace: {
    icon: <LayoutGrid className="size-3" />,
    itemName: (workspace) => workspace?.name,
    path: (workspace) => `/${workspace?.slug}/`,
    title: "Workspaces",
  },
};
