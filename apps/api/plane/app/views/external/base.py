...
class RephraseGrammarEndpoint(BaseAPIView):
    """
    Workspace-scoped endpoint for editor AI tasks.

    Accepts:
        text_input  (required) — selected text from the editor
        prompt      (optional) — freeform user instruction (ASK_ANYTHING flow)
        casual_score / formal_score (optional, int 0-10) — tone weighting
    Returns:
    ...
    def post(self, request, slug):
        ...
        task_parts = [
            "You are a writing assistant helping improve text in a project management tool.",
        ]

        if user_prompt:
            task_parts.append(f"User instruction: {user_prompt}")
        else:
            task_parts.append(
        ...
        try:
            if casual_score is not None and formal_score is not None:
                casual = int(casual_score)
                formal = int(formal_score)
                if casual > formal:
                    task_parts.append("Use a casual, friendly tone.")
                elif formal > casual:
                    task_parts.append("Use a formal, professional tone.")
        except (TypeError, ValueError):
            pass  # Malformed scores are silently ignored; tone defaults to neutral

        task_parts.append(
            "Return only the improved text without any preamble, explanation, or markdown."
        )
        task = " ".join(task_parts)

        text, error = get_llm_response(task, text_input, api_key, model, provider)
        if error or not text:
            return Response(
                {"error": "Failed to generate a response from the AI provider"},
