...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let configs: &[(&str, Option<&str>, &str, bool)] = &[
            ...
            ("ENABLE_MAGIC_LINK_LOGIN",  Some("0"),                 "AUTHENTICATION", false),
            // WORKSPACE_MANAGEMENT
            ("DISABLE_WORKSPACE_CREATION", Some("0"),              "WORKSPACE_MANAGEMENT", false),
            // GOOGLE
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        ...
        let keys: &[&str] = &[
            ...
            "IS_GOOGLE_ENABLED", "GOOGLE_CLIENT_ID", "GOOGLE_CLIENT_SECRET", "ENABLE_GOOGLE_SYNC",
            "IS_GITHUB_ENABLED", "GITHUB_CLIENT_ID", "GITHUB_CLIENT_SECRET", "GITHUB_ORGANIZATION_ID",
            "ENABLE_GITHUB_SYNC", "IS_GITHUB_INTEGRATION_ENABLED", "GITHUB_APP_NAME", "GITHUB_APP_ID",
            "GITHUB_APP_PRIVATE_KEY", "GITHUB_WEBHOOK_SECRET",
            "IS_GITLAB_ENABLED", "IS_GITLAB_INTEGRATION_ENABLED", "GITLAB_HOST",
            "GITLAB_CLIENT_ID", "GITLAB_CLIENT_SECRET", "ENABLE_GITLAB_SYNC",
            "IS_GITEA_ENABLED", "GITEA_HOST", "GITEA_CLIENT_ID", "GITEA_CLIENT_SECRET", "ENABLE_GITEA_SYNC",
            "IS_SLACK_ENABLED", "SLACK_CLIENT_ID", "SLACK_CLIENT_SECRET",
            "ENABLE_SMTP", "EMAIL_HOST", "EMAIL_HOST_USER", "EMAIL_HOST_PASSWORD",
            "EMAIL_PORT", "EMAIL_FROM", "EMAIL_USE_TLS", "EMAIL_USE_SSL",
            "LLM_API_KEY", "LLM_PROVIDER", "LLM_MODEL", "GPT_ENGINE",
            "UNSPLASH_ACCESS_KEY",
            "IS_INTERCOM_ENABLED", "INTERCOM_APP_ID",
        ];

        let keys_sql = keys.iter().map(|k| format!("'{k}'")).collect::<Vec<_>>().join(", ");
        db.execute_unprepared(&format!(
