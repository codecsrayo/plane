...
async fn require_instance_admin(state: &AppState, user: &users::Model) -> Result<(), AppError> {
    if user.is_superuser { return Ok(()); }
...
async fn cfg(state: &AppState, key: &str, fallback: &str) -> String {
    get_config_value(state, key, Some(fallback))
        .await.unwrap_or_default().unwrap_or_else(|| fallback.to_owned())
}

async fn serialize_instance(state: &AppState, inst: &instances::Model) -> Value {
    let workspaces_exist = workspaces::Entity::find().active()
        .count(&state.db).await.unwrap_or(0) >= 1;
    json!({
        "id": inst.id, "instance_name": inst.instance_name,
        "instance_id": inst.instance_id, "current_version": inst.current_version,
        "latest_version": inst.latest_version, "namespace": inst.namespace,
        "whitelist_emails": inst.whitelist_emails, "domain": inst.domain,
        "edition": inst.edition, "is_setup_done": inst.is_setup_done,
        ...
        "is_verified": inst.is_verified, "is_test": inst.is_test,
        ...
        "is_activated": true, "workspaces_exist": workspaces_exist,
    })
}
...
async fn build_config(state: &AppState) -> Value {
    ...
    json!({
        ...
        "github_app_name": github_app, "github_client_id": github_cid,
        "gitlab_client_id": gitlab_cid, "gitlab_host": gitlab_host,
        "slack_client_id": slack_cid,
        "posthog_api_key": posthog_key, "posthog_host": posthog_host,
        ...
        "is_intercom_enabled": intercom_en == "1", "intercom_app_id": intercom_id,
...
fn serialize_config_row(c: &instance_configurations::Model) -> Value {
    ...
    json!({
        "id": c.id, "key": c.key, "value": decrypted,
        "category": c.category, "is_encrypted": c.is_encrypted,
...
// NOTE: utoipa requires ToSchema on any type used in Json<T> inside a path handler.
//       We derive it here to satisfy the macro without having to register every
//       struct in the global OpenApi components list.
...
pub async fn patch_instance(
    ...
    let updated: instances::Model = active.update(&state.db).await.map_err(AppError::Database)?;
...
#[utoipa::path(get, path = "/instances/admins/", tag = "Instance",
    security(("TokenAuth" = [])),
    responses((status = 200, description = "List of admins")))]
...
pub async fn email_credentials_check(
    ...
    let email = Message::builder()
        .from(email_from.parse()
            .map_err(|_| AppError::BadRequest("Invalid EMAIL_FROM".into()))?)
    ...
    if !use_tls { builder = builder.tls(Tls::None); }
