...
| **`api_rust/issues` `reject_if_rich_filters` falso positivo:** el guard introducido en `10bf8ae` rechazaba cualquier string no vacío, incluyendo `filters={}` (URL-encoded `%7B%7D`). El frontend manda `filters={}` en ciertos layouts (gantt_chart, spreadsheet) como parte estructural del query-building aunque no haya filtros activos → `400 Bad Request` en prod rompía el render del gantt chart. Fix: parsear el param como JSON y aceptar formas vacías (`{}`, `[]`, `null`, ausente, string vacío) como no-op; rechazar solo cuando hay contenido real o JSON malformado. | ✅ corregido |
| **`api_rust/jobs/export` — /settings/exports descargaba un comprimido ilegible en vez del archivo:** el worker generaba un buffer custom (bloques `flate2::GzEncoder` concatenados con prefijos de 8 bytes little-endian con la longitud), lo subía como `.tar.gz` con `Content-Type: application/gzip`, e ignoraba completamente `exporter.provider` (siempre emitía CSV hardcodeado). Ningún descompresor estándar podía abrir el archivo — el SO lo reconocía como comprimido solo porque los primeros bytes parecían gzip. Fix: reescritura completa con paridad `bgtasks/export_task.py` — ZIP real con `zip::ZipWriter`+`CompressionMethod::Deflated`, branching por `provider` (csv/json/xlsx) usando `csv` crate, `serde_json::to_string_pretty` y `rust_xlsxwriter::Workbook`, respeto del flag `multiple` (single consolidado vs file-per-project), filename S3 `{workspace_id}/export-{slug}-{token[:6]}-{YYYY-MM-DD}.zip`, `Content-Type: application/zip`, estado `"processing"` al inicio del job (faltaba). Deps nuevas: `zip = "2.2"`, `rust_xlsxwriter = "0.94"`, `csv = "1.3"`. | ✅ corregido (`c7868ea`) |
| **`api_rust/jobs/export` — filenames dentro del ZIP indistinguibles entre proyectos:** Django (y mi commit inicial de paridad) nombraba los archivos como `{slug}-{project_uuid}.{ext}`. Exportando varios proyectos, todos los archivos compartían el prefijo `{slug}-` y se diferenciaban solo por UUID — visualmente idénticos a los usuarios. Fix: batch-fetch de `projects` (anti-N+1) para obtener `identifier` + `name` por `project_id`, y construir filename `{slug}-{IDENT}-{name_sanitizado}.{ext}` (ej. `test-FRONT-web-platform.xlsx`). Helpers nuevos: `sanitize_filename_segment` (colapsa caracteres no-seguros a `-`, trim, max 64 chars), `project_label` (combina identifier + name con fallback), `unique_base_name` (apendea `-2`/`-3` si hubiera colisión). Fallback a UUID truncado a 8 chars si el proyecto se borró entre enqueue y ejecución. El caso `multiple=false` pasa de `{slug}-{workspace_uuid}` a `{slug}-all-projects` por la misma razón de legibilidad. | ✅ corregido |
