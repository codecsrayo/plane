---
source_file: "store/issue/workspace/issue.store.ts"
type: "code"
community: "Filters Params"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Filters_Params
---

# issue.store.ts

## Connections

- [[IBaseIssuesStore]] - `imports` [EXTRACTED]
- [[IIssueRootStore]] - `imports` [EXTRACTED]
- [[IWorkspaceIssues]] - `contains` [EXTRACTED]
- [[IWorkspaceIssuesFilter]] - `imports` [EXTRACTED]
- [[WorkspaceIssues]] - `contains` [EXTRACTED]
- [[WorkspaceService]] - `imports` [EXTRACTED]
- [[api.service.ts]] - `imports_from` [EXTRACTED]
- [[base-issues.store.ts]] - `imports_from` [EXTRACTED]
- [[cursorObject_2]] - `contains` [EXTRACTED]
- [[filter.store.ts_2]] - `imports_from` [EXTRACTED]
- [[params_2]] - `contains` [EXTRACTED]
- [[root.store.ts_1]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[workspace.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Filters_Params
