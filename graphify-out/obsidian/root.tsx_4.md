---
source_file: "editor/src/core/extensions/slash-commands/root.tsx"
type: "code"
community: "Emoji Command"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Emoji_Command
---

# root.tsx

## Connections

- [[CORE_EXTENSIONS]] - `imports` [EXTRACTED]
- [[Command]] - `contains` [EXTRACTED]
- [[CommandListInstance]] - `imports` [EXTRACTED]
- [[DROPDOWN_NAVIGATION_KEYS]] - `imports` [EXTRACTED]
- [[SlashCommandOptions]] - `contains` [EXTRACTED]
- [[SlashCommands()]] - `contains` [EXTRACTED]
- [[SlashCommandsMenu]] - `imports` [EXTRACTED]
- [[SlashCommandsMenuProps]] - `imports` [EXTRACTED]
- [[TExtensionProps]] - `contains` [EXTRACTED]
- [[TSlashCommandAdditionalOption]] - `contains` [EXTRACTED]
- [[UpdateFloatingUIFloaterPosition]] - `imports` [EXTRACTED]
- [[command-items-list.tsx]] - `imports_from` [EXTRACTED]
- [[command-menu.tsx]] - `imports_from` [EXTRACTED]
- [[extension.ts]] - `imports_from` [EXTRACTED]
- [[floating-ui.ts]] - `imports_from` [EXTRACTED]
- [[getSlashCommandFilteredSections()]] - `imports` [EXTRACTED]
- [[index.ts_46]] - `imports_from` [EXTRACTED]
- [[noopCleanup()_1]] - `contains` [EXTRACTED]
- [[rich-text-extensions.tsx]] - `imports_from` [EXTRACTED]
- [[tippy.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Emoji_Command
