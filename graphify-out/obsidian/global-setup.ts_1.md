---
source_file: "packages/e2e-utils/src/setup/global-setup.ts"
type: "code"
community: "Community 174"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_174
---

# global-setup.ts

## Connections
- [[api.ts_1]] - `imports_from` [EXTRACTED]
- [[createProject()]] - `imports` [EXTRACTED]
- [[createWorkspace()]] - `imports` [EXTRACTED]
- [[ensureInstanceConfigured()]] - `imports` [EXTRACTED]
- [[fetchCsrfToken()]] - `imports` [EXTRACTED]
- [[getStates()]] - `imports` [EXTRACTED]
- [[globalSetup()]] - `contains` [EXTRACTED]
- [[signIn()]] - `imports` [EXTRACTED]
- [[signUp()]] - `imports` [EXTRACTED]
- [[waitForHealth()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_174