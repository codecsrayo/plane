---
source_file: "editor/src/core/extensions/custom-link/extension.tsx"
type: "code"
community: "Community 113"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_113
---

# extension.tsx

## Connections
- [[CORE_EXTENSIONS]] - `imports` [EXTRACTED]
- [[Commands_4]] - `contains` [EXTRACTED]
- [[CustomLinkExtension]] - `contains` [EXTRACTED]
- [[CustomLinkStorage]] - `contains` [EXTRACTED]
- [[LinkOptions]] - `contains` [EXTRACTED]
- [[LinkProtocolOptions]] - `contains` [EXTRACTED]
- [[Storage_3]] - `contains` [EXTRACTED]
- [[autolink()]] - `imports` [EXTRACTED]
- [[autolink.ts]] - `imports_from` [EXTRACTED]
- [[clickHandler()]] - `imports` [EXTRACTED]
- [[clickHandler.ts]] - `imports_from` [EXTRACTED]
- [[common.ts_1]] - `imports_from` [EXTRACTED]
- [[extension.ts]] - `imports_from` [EXTRACTED]
- [[isValidHttpUrl()]] - `imports` [EXTRACTED]
- [[pasteHandler()]] - `imports` [EXTRACTED]
- [[pasteHandler.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_113