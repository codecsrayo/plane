---
source_file: "web/core/components/gantt-chart/data/index.ts"
type: "code"
community: "Time Month"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Time_Month
---

# index.ts

## Connections

- [[VIEWS_LIST]] - `contains` [EXTRACTED]
- [[base-timeline.store.ts]] - `imports_from` [EXTRACTED]
- [[bindZero()]] - `contains` [EXTRACTED]
- [[charCapitalize()]] - `contains` [EXTRACTED]
- [[currentViewDataWithView()]] - `contains` [EXTRACTED]
- [[datePreview()]] - `contains` [EXTRACTED]
- [[generateWeeks()]] - `contains` [EXTRACTED]
- [[header.tsx_43]] - `imports_from` [EXTRACTED]
- [[issue_gantt_view.store.ts]] - `imports_from` [EXTRACTED]
- [[month-view.ts]] - `imports_from` [EXTRACTED]
- [[months]] - `contains` [EXTRACTED]
- [[quarter-view.ts]] - `imports_from` [EXTRACTED]
- [[quarters]] - `contains` [EXTRACTED]
- [[root.tsx_48]] - `imports_from` [EXTRACTED]
- [[timePreview()]] - `contains` [EXTRACTED]
- [[week-view.ts]] - `imports_from` [EXTRACTED]
- [[weeks_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Time_Month
