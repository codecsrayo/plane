---
source_file: "web/core/hooks/store/use-project-state.ts"
type: "code"
community: "Work Item Detail & Layouts"
location: "L13"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Work_Item_Detail__Layouts
---

# useProjectState()

## Connections
- [[all-issue.tsx]] - `imports` [EXTRACTED]
- [[all-properties.tsx]] - `imports` [EXTRACTED]
- [[auto-close-automation.tsx]] - `imports` [EXTRACTED]
- [[blocks.tsx_1]] - `imports` [EXTRACTED]
- [[change-state-list.tsx]] - `imports` [EXTRACTED]
- [[cycle-issue.tsx]] - `imports` [EXTRACTED]
- [[draft-issue-properties.tsx]] - `imports` [EXTRACTED]
- [[dropdown.tsx_3]] - `imports` [EXTRACTED]
- [[dropdown.tsx_5]] - `imports` [EXTRACTED]
- [[due-date-column.tsx]] - `imports` [EXTRACTED]
- [[form.tsx_8]] - `imports` [EXTRACTED]
- [[issue-block.tsx]] - `imports` [EXTRACTED]
- [[issue-detail.tsx]] - `imports` [EXTRACTED]
- [[issue.tsx]] - `imports` [EXTRACTED]
- [[kanban-group.tsx]] - `imports` [EXTRACTED]
- [[list-group.tsx]] - `imports` [EXTRACTED]
- [[module-issue.tsx]] - `imports` [EXTRACTED]
- [[pr-state-mapping-modal.tsx]] - `imports` [EXTRACTED]
- [[pr-state-mapping.tsx]] - `imports` [EXTRACTED]
- [[priority-chart.tsx]] - `imports` [EXTRACTED]
- [[project-issue-sync-modal.tsx]] - `imports` [EXTRACTED]
- [[project-issue.tsx]] - `imports` [EXTRACTED]
- [[project-level.tsx_1]] - `imports` [EXTRACTED]
- [[project-wrapper.tsx]] - `imports` [EXTRACTED]
- [[properties.tsx]] - `imports` [EXTRACTED]
- [[properties.tsx_2]] - `imports` [EXTRACTED]
- [[root.tsx_49]] - `imports` [EXTRACTED]
- [[root.tsx_68]] - `imports` [EXTRACTED]
- [[root.tsx_76]] - `imports` [EXTRACTED]
- [[sidebar.tsx_4]] - `imports` [EXTRACTED]
- [[state-delete-modal.tsx]] - `imports` [EXTRACTED]
- [[state-item-title.tsx]] - `imports` [EXTRACTED]
- [[state.tsx_5]] - `imports` [EXTRACTED]
- [[state.tsx_6]] - `imports` [EXTRACTED]
- [[states-menu.tsx]] - `imports` [EXTRACTED]
- [[title-actions.tsx]] - `imports` [EXTRACTED]
- [[use-project-issue-properties.ts]] - `imports` [EXTRACTED]
- [[use-project-state.ts]] - `contains` [EXTRACTED]
- [[use-work-item-filters-config.tsx]] - `imports` [EXTRACTED]
- [[useProjectIssueProperties()]] - `calls` [EXTRACTED]
- [[useWorkItemFiltersConfig()]] - `calls` [EXTRACTED]
- [[workspace-wrapper.tsx]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Work_Item_Detail__Layouts