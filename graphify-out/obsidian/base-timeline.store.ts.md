---
source_file: "web/ce/store/timeline/base-timeline.store.ts"
type: "code"
community: "Time Month"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Time_Month
---

# base-timeline.store.ts

## Connections

- [[BaseTimeLineStore]] - `contains` [EXTRACTED]
- [[BlockData]] - `contains` [EXTRACTED]
- [[IBaseTimelineStore]] - `contains` [EXTRACTED]
- [[TGanttRenderPayload]] - `contains` [EXTRACTED]
- [[currentViewDataWithView()]] - `imports` [EXTRACTED]
- [[getDateFromPositionOnGantt()]] - `imports` [EXTRACTED]
- [[getItemPositionWidth()]] - `imports` [EXTRACTED]
- [[getPositionFromDate()]] - `imports` [EXTRACTED]
- [[helpers.ts_2]] - `imports_from` [EXTRACTED]
- [[index.ts_209]] - `imports_from` [EXTRACTED]
- [[index.ts_311]] - `imports_from` [EXTRACTED]
- [[index.ts_307]] - `imports_from` [EXTRACTED]
- [[newCurrentViewData]] - `contains` [EXTRACTED]
- [[updatePayload]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Time_Month
