---
source_file: "web/core/store/issue/project/issue.store.ts"
type: "code"
community: "Community 33"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_33
---

# issue.store.ts

## Connections
- [[IBaseIssuesStore]] - `imports` [EXTRACTED]
- [[IIssueRootStore]] - `imports` [EXTRACTED]
- [[IProjectIssues]] - `contains` [EXTRACTED]
- [[IProjectIssuesFilter]] - `imports` [EXTRACTED]
- [[ProjectIssues]] - `contains` [EXTRACTED]
- [[api.service.ts_1]] - `imports_from` [EXTRACTED]
- [[base-issues.store.ts]] - `imports_from` [EXTRACTED]
- [[cursorObject_5]] - `contains` [EXTRACTED]
- [[filter.store.ts_11]] - `imports_from` [EXTRACTED]
- [[params_6]] - `contains` [EXTRACTED]
- [[root.store.ts_3]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_33