---
source_file: "web/core/components/issues/peek-overview/view.tsx"
type: "code"
community: "Activity & Inbox Filters"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Activity__Inbox_Filters
---

# view.tsx

## Connections
- [[IIssueView]] - `contains` [EXTRACTED]
- [[IssuePeekOverviewError()]] - `imports` [EXTRACTED]
- [[IssuePeekOverviewHeader]] - `imports` [EXTRACTED]
- [[IssuePeekOverviewLoader()]] - `imports` [EXTRACTED]
- [[IssueView]] - `contains` [EXTRACTED]
- [[PeekOverviewIssueDetails]] - `imports` [EXTRACTED]
- [[PeekOverviewProperties]] - `imports` [EXTRACTED]
- [[TPeekModes]] - `imports` [EXTRACTED]
- [[error.tsx]] - `imports_from` [EXTRACTED]
- [[header.tsx_51]] - `imports_from` [EXTRACTED]
- [[index.ts_367]] - `imports_from` [EXTRACTED]
- [[index.ts_368]] - `imports_from` [EXTRACTED]
- [[index.ts_377]] - `imports_from` [EXTRACTED]
- [[issue-detail.tsx_1]] - `imports_from` [EXTRACTED]
- [[loader.tsx_8]] - `imports_from` [EXTRACTED]
- [[properties.tsx_2]] - `imports_from` [EXTRACTED]
- [[root.tsx_84]] - `imports_from` [EXTRACTED]
- [[use-issue-detail.ts]] - `imports_from` [EXTRACTED]
- [[use-keypress.tsx]] - `imports_from` [EXTRACTED]
- [[use-peek-overview-outside-click.tsx]] - `imports_from` [EXTRACTED]
- [[useIssueDetail()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Activity__Inbox_Filters