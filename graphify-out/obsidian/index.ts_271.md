---
source_file: "web/core/store/workspace/index.ts"
type: "code"
community: "Community 129"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_129
---

# index.ts

## Connections
- [[CoreRootStore]] - `imports` [EXTRACTED]
- [[HomeStore]] - `imports` [EXTRACTED]
- [[IHomeStore]] - `imports` [EXTRACTED]
- [[IWebhookStore]] - `imports` [EXTRACTED]
- [[IWorkspaceRootStore]] - `contains` [EXTRACTED]
- [[WebhookStore]] - `imports` [EXTRACTED]
- [[WorkspaceService_1]] - `imports` [EXTRACTED]
- [[api.service.ts_1]] - `imports_from` [EXTRACTED]
- [[beforeUpdateData]] - `contains` [EXTRACTED]
- [[constructor()_6]] - `contains` [EXTRACTED]
- [[currentWorkspace()]] - `contains` [EXTRACTED]
- [[home.ts_1]] - `imports_from` [EXTRACTED]
- [[index.ts_204]] - `imports_from` [EXTRACTED]
- [[isCurrentWorkspaceValid]] - `contains` [EXTRACTED]
- [[root.store.ts_2]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[updatedPreferences]] - `contains` [EXTRACTED]
- [[use-workspace.ts]] - `imports_from` [EXTRACTED]
- [[webhook.store.ts]] - `imports_from` [EXTRACTED]
- [[workspace.service.ts_1]] - `imports_from` [EXTRACTED]
- [[workspacesCreatedByCurrentUser()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_129