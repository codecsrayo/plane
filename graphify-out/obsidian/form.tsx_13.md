---
source_file: "admin/app/(all)/(dashboard)/integrations/form.tsx"
type: "code"
community: "Instance Config"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Instance_Config
---

# form.tsx

## Connections

- [[CodeBlock()]] - `imports` [EXTRACTED]
- [[ConfirmDiscardModal()]] - `imports` [EXTRACTED]
- [[ControllerInput()]] - `imports` [EXTRACTED]
- [[CopyField()]] - `imports` [EXTRACTED]
- [[InstanceIntegrationsConfigForm]] - `contains` [EXTRACTED]
- [[IntegrationConfigFormValues]] - `contains` [EXTRACTED]
- [[Props_540]] - `contains` [EXTRACTED]
- [[TControllerInputFormField]] - `imports` [EXTRACTED]
- [[code-block.tsx]] - `imports_from` [EXTRACTED]
- [[config.ts_5]] - `imports_from` [EXTRACTED]
- [[confirm-discard-modal.tsx]] - `imports_from` [EXTRACTED]
- [[controller-input.tsx]] - `imports_from` [EXTRACTED]
- [[copy-field.tsx]] - `imports_from` [EXTRACTED]
- [[decodeBase64()]] - `contains` [EXTRACTED]
- [[decodePrivateKeyForDisplay()]] - `contains` [EXTRACTED]
- [[encodeBase64()]] - `contains` [EXTRACTED]
- [[encodePrivateKeyForStorage()]] - `contains` [EXTRACTED]
- [[index.ts_445]] - `imports_from` [EXTRACTED]
- [[normalizePemBlock()]] - `contains` [EXTRACTED]
- [[page.tsx_71]] - `imports_from` [EXTRACTED]
- [[toBool()]] - `imports` [EXTRACTED]
- [[use-origin.ts]] - `imports_from` [EXTRACTED]
- [[useOrigin()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Instance_Config
