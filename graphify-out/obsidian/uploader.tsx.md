---
source_file: "editor/src/core/extensions/custom-image/components/uploader.tsx"
type: "code"
community: "Community 28"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_28
---

# uploader.tsx

## Connections
- [[ACCEPTED_IMAGE_MIME_TYPES]] - `imports` [EXTRACTED]
- [[CORE_EXTENSIONS]] - `imports` [EXTRACTED]
- [[CustomImageNodeViewProps]] - `imports` [EXTRACTED]
- [[CustomImageUploader()]] - `contains` [EXTRACTED]
- [[CustomImageUploaderProps]] - `contains` [EXTRACTED]
- [[ECustomImageStatus]] - `imports` [EXTRACTED]
- [[EFileError]] - `imports` [EXTRACTED]
- [[config.ts_1]] - `imports_from` [EXTRACTED]
- [[extension.ts]] - `imports_from` [EXTRACTED]
- [[file.ts]] - `imports_from` [EXTRACTED]
- [[getImageComponentImageFileMap()]] - `imports` [EXTRACTED]
- [[node-view.tsx]] - `imports_from` [EXTRACTED]
- [[types.ts_6]] - `imports_from` [EXTRACTED]
- [[uploadFirstFileAndInsertRemaining()]] - `imports` [EXTRACTED]
- [[use-file-upload.ts]] - `imports_from` [EXTRACTED]
- [[useDropZone()]] - `imports` [EXTRACTED]
- [[useUploader()]] - `imports` [EXTRACTED]
- [[utils.ts_9]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_28