---
source_file: "web/core/hooks/use-multiple-select.ts"
type: "code"
community: "Select Multiple"
location: "L29"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Select_Multiple
---

# TSelectionHelper

## Connections

- [[block-root.tsx]] - `imports` [EXTRACTED]
- [[block-row-list.tsx]] - `imports` [EXTRACTED]
- [[block-row.tsx]] - `imports` [EXTRACTED]
- [[block.tsx_4]] - `imports` [EXTRACTED]
- [[block.tsx_7]] - `imports` [EXTRACTED]
- [[blocks-list.tsx_2]] - `imports` [EXTRACTED]
- [[entity-select-action.tsx]] - `imports` [EXTRACTED]
- [[group-by-card.tsx_1]] - `imports` [EXTRACTED]
- [[group-select-action.tsx]] - `imports` [EXTRACTED]
- [[issue-row.tsx]] - `imports` [EXTRACTED]
- [[list-group.tsx]] - `imports` [EXTRACTED]
- [[root.tsx_29]] - `imports` [EXTRACTED]
- [[root.tsx_47]] - `imports` [EXTRACTED]
- [[select-group.tsx]] - `imports` [EXTRACTED]
- [[sidebar.tsx_2]] - `imports` [EXTRACTED]
- [[spreadsheet-header.tsx]] - `imports` [EXTRACTED]
- [[spreadsheet-table.tsx]] - `imports` [EXTRACTED]
- [[use-multiple-select.ts]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Select_Multiple
