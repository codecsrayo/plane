---
source_file: "web/core/store/member/workspace/workspace-member.store.ts"
type: "code"
community: "Community 150"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_150
---

# workspace-member.store.ts

## Connections

- [[IMemberRootStore]] - `imports` [EXTRACTED]
- [[IRouterStore]] - `imports` [EXTRACTED]
- [[IUserStore]] - `imports` [EXTRACTED]
- [[IWorkspaceMemberFiltersStore]] - `imports` [EXTRACTED]
- [[IWorkspaceMemberStore]] - `contains` [EXTRACTED]
- [[IWorkspaceMembership_1]] - `contains` [EXTRACTED]
- [[WorkspaceMemberFiltersStore]] - `imports` [EXTRACTED]
- [[WorkspaceMemberStore]] - `contains` [EXTRACTED]
- [[WorkspaceService]] - `imports` [EXTRACTED]
- [[api.service.ts]] - `imports_from` [EXTRACTED]
- [[filteredMemberIds]] - `contains` [EXTRACTED]
- [[index.ts_17]] - `imports_from` [EXTRACTED]
- [[index.ts_7]] - `imports_from` [EXTRACTED]
- [[invitation]] - `contains` [EXTRACTED]
- [[memberDetails]] - `contains` [EXTRACTED]
- [[memberIds]] - `contains` [EXTRACTED]
- [[memberInvitations]] - `contains` [EXTRACTED]
- [[members]] - `contains` [EXTRACTED]
- [[originalMemberInvitations]] - `contains` [EXTRACTED]
- [[originalProjectMemberData]] - `contains` [EXTRACTED]
- [[root.store.ts_1]] - `imports_from` [EXTRACTED]
- [[router.store.ts]] - `imports_from` [EXTRACTED]
- [[searchedWorkspaceMemberIds]] - `contains` [EXTRACTED]
- [[searchedWorkspaceMemberInvitationIds]] - `contains` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[workspace-member-filters.store.ts]] - `imports_from` [EXTRACTED]
- [[workspace.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_150
