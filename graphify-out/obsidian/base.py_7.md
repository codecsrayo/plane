---
source_file: "api/plane/app/views/external/base.py"
type: "code"
community: "Provider Endpoint"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Provider_Endpoint
---

# base.py

## Connections

- [[AnthropicProvider]] - `contains` [EXTRACTED]
- [[GPTIntegrationEndpoint]] - `contains` [EXTRACTED]
- [[GeminiProvider]] - `contains` [EXTRACTED]
- [[LLMProvider]] - `contains` [EXTRACTED]
- [[OpenAIProvider]] - `contains` [EXTRACTED]
- [[RephraseGrammarEndpoint]] - `contains` [EXTRACTED]
- [[UnsplashEndpoint]] - `contains` [EXTRACTED]
- [[WorkspaceGPTIntegrationEndpoint]] - `contains` [EXTRACTED]
- [[__init__.py_3]] - `imports_from` [EXTRACTED]
- [[base.py]] - `imports_from` [EXTRACTED]
- [[get_config()]] - `contains` [EXTRACTED]
- [[get_llm_config()]] - `contains` [EXTRACTED]
- [[get_llm_response()]] - `contains` [EXTRACTED]
- [[post()_9]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Provider_Endpoint
