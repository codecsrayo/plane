---
source_file: "editor/src/core/extensions/table/plugins/drag-handles/actions.ts"
type: "code"
community: "Community 38"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_38
---

# actions.ts

## Connections
- [[TableNodeLocation]] - `imports` [EXTRACTED]
- [[TableRow_1]] - `contains` [EXTRACTED]
- [[TableRows]] - `contains` [EXTRACTED]
- [[drag-handle.tsx_1]] - `imports_from` [EXTRACTED]
- [[drag-handle.tsx_2]] - `imports_from` [EXTRACTED]
- [[dropdown.tsx]] - `imports_from` [EXTRACTED]
- [[dropdown.tsx_1]] - `imports_from` [EXTRACTED]
- [[duplicateColumns()]] - `contains` [EXTRACTED]
- [[duplicateRows()]] - `contains` [EXTRACTED]
- [[helpers.ts]] - `imports_from` [EXTRACTED]
- [[moveSelectedColumns()]] - `contains` [EXTRACTED]
- [[moveSelectedRows()]] - `contains` [EXTRACTED]
- [[tableFromCells()]] - `contains` [EXTRACTED]
- [[tableToCells()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_38