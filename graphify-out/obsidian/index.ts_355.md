---
source_file: "web/core/components/issues/issue-layouts/filters/index.ts"
type: "code"
community: "Issue Display Filters"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Issue_Display_Filters
---

# index.ts

## Connections
- [[access.tsx_1]] - `imports_from` [EXTRACTED]
- [[archived-issues-header.tsx]] - `imports_from` [EXTRACTED]
- [[assignee.tsx]] - `imports_from` [EXTRACTED]
- [[created-at.tsx]] - `imports_from` [EXTRACTED]
- [[created-at.tsx_1]] - `imports_from` [EXTRACTED]
- [[created-by.tsx]] - `imports_from` [EXTRACTED]
- [[created-by.tsx_1]] - `imports_from` [EXTRACTED]
- [[cycle.tsx_2]] - `imports_from` [EXTRACTED]
- [[cycles-view-header.tsx]] - `imports_from` [EXTRACTED]
- [[date.tsx_7]] - `imports_from` [EXTRACTED]
- [[display-filters-selection.tsx]] - `imports_from` [EXTRACTED]
- [[display-filters.tsx]] - `imports_from` [EXTRACTED]
- [[due-date.tsx]] - `imports_from` [EXTRACTED]
- [[end-date.tsx]] - `imports_from` [EXTRACTED]
- [[extra-options.tsx]] - `imports_from` [EXTRACTED]
- [[filter-selection.tsx]] - `imports_from` [EXTRACTED]
- [[filters.tsx_1]] - `imports_from` [EXTRACTED]
- [[filters.tsx_2]] - `imports_from` [EXTRACTED]
- [[filters.tsx_3]] - `imports_from` [EXTRACTED]
- [[form.tsx]] - `imports_from` [EXTRACTED]
- [[form.tsx_1]] - `imports_from` [EXTRACTED]
- [[group-by.tsx]] - `imports_from` [EXTRACTED]
- [[header.tsx_5]] - `imports_from` [EXTRACTED]
- [[header.tsx_7]] - `imports_from` [EXTRACTED]
- [[header.tsx_12]] - `imports_from` [EXTRACTED]
- [[header.tsx_17]] - `imports_from` [EXTRACTED]
- [[header.tsx_44]] - `imports_from` [EXTRACTED]
- [[header.tsx_52]] - `imports_from` [EXTRACTED]
- [[labels.tsx_1]] - `imports_from` [EXTRACTED]
- [[labels.tsx_3]] - `imports_from` [EXTRACTED]
- [[lead.tsx]] - `imports_from` [EXTRACTED]
- [[lead.tsx_1]] - `imports_from` [EXTRACTED]
- [[member-list.tsx_1]] - `imports_from` [EXTRACTED]
- [[members.tsx_2]] - `imports_from` [EXTRACTED]
- [[members.tsx_4]] - `imports_from` [EXTRACTED]
- [[members.tsx_9]] - `imports_from` [EXTRACTED]
- [[mentions.tsx]] - `imports_from` [EXTRACTED]
- [[mobile-header.tsx]] - `imports_from` [EXTRACTED]
- [[mobile-header.tsx_1]] - `imports_from` [EXTRACTED]
- [[mobile-header.tsx_2]] - `imports_from` [EXTRACTED]
- [[mobile-header.tsx_4]] - `imports_from` [EXTRACTED]
- [[mobile-header.tsx_5]] - `imports_from` [EXTRACTED]
- [[mobile-header.tsx_7]] - `imports_from` [EXTRACTED]
- [[module-view-header.tsx]] - `imports_from` [EXTRACTED]
- [[module.tsx_2]] - `imports_from` [EXTRACTED]
- [[order-by.tsx_3]] - `imports_from` [EXTRACTED]
- [[priority.tsx_2]] - `imports_from` [EXTRACTED]
- [[priority.tsx_5]] - `imports_from` [EXTRACTED]
- [[profile-issues-filter.tsx]] - `imports_from` [EXTRACTED]
- [[project.tsx_4]] - `imports_from` [EXTRACTED]
- [[root.tsx_55]] - `imports_from` [EXTRACTED]
- [[root.tsx_66]] - `imports_from` [EXTRACTED]
- [[root.tsx_113]] - `imports_from` [EXTRACTED]
- [[root.tsx_119]] - `imports_from` [EXTRACTED]
- [[root.tsx_124]] - `imports_from` [EXTRACTED]
- [[start-date.tsx]] - `imports_from` [EXTRACTED]
- [[start-date.tsx_1]] - `imports_from` [EXTRACTED]
- [[start-date.tsx_2]] - `imports_from` [EXTRACTED]
- [[state-group.tsx]] - `imports_from` [EXTRACTED]
- [[state.tsx_1]] - `imports_from` [EXTRACTED]
- [[state.tsx_4]] - `imports_from` [EXTRACTED]
- [[status.tsx_1]] - `imports_from` [EXTRACTED]
- [[status.tsx_3]] - `imports_from` [EXTRACTED]
- [[status.tsx_5]] - `imports_from` [EXTRACTED]
- [[sub-group-by.tsx]] - `imports_from` [EXTRACTED]
- [[target-date.tsx]] - `imports_from` [EXTRACTED]
- [[view-list-header.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Issue_Display_Filters