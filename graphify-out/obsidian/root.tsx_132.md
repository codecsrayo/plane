---
source_file: "web/core/components/rich-filters/filter-item/root.tsx"
type: "code"
community: "Value Input"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Value_Input
---

# root.tsx

## Connections

- [[FilterItem]] - `contains` [EXTRACTED]
- [[FilterItemCloseButton]] - `imports` [EXTRACTED]
- [[FilterItemContainer()]] - `imports` [EXTRACTED]
- [[FilterItemLoader()]] - `imports` [EXTRACTED]
- [[FilterItemProperty]] - `imports` [EXTRACTED]
- [[FilterValueInput]] - `imports` [EXTRACTED]
- [[IFilterItemProps]] - `contains` [EXTRACTED]
- [[InvalidFilterItem]] - `imports` [EXTRACTED]
- [[close-button.tsx]] - `imports_from` [EXTRACTED]
- [[container.tsx]] - `imports_from` [EXTRACTED]
- [[filters-row.tsx_1]] - `imports_from` [EXTRACTED]
- [[invalid.tsx]] - `imports_from` [EXTRACTED]
- [[loader.tsx_11]] - `imports_from` [EXTRACTED]
- [[property.tsx]] - `imports_from` [EXTRACTED]
- [[root.tsx_133]] - `imports_from` [EXTRACTED]
- [[shared.ts_14]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Value_Input
