---
source_file: "propel/src/dialog/dialog.stories.tsx"
type: "code"
community: "Dialog Width"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Dialog_Width
---

# dialog.stories.tsx

## Connections

- [[AllWidths]] - `contains` [EXTRACTED]
- [[CloseIcon()]] - `imports` [EXTRACTED]
- [[ConfirmationDialog]] - `contains` [EXTRACTED]
- [[Default_14]] - `contains` [EXTRACTED]
- [[Dialog]] - `imports` [EXTRACTED]
- [[EDialogWidth]] - `imports` [EXTRACTED]
- [[FormDialog]] - `contains` [EXTRACTED]
- [[LargeWidth]] - `contains` [EXTRACTED]
- [[ScrollableContent]] - `contains` [EXTRACTED]
- [[SmallWidth]] - `contains` [EXTRACTED]
- [[Story_15]] - `contains` [EXTRACTED]
- [[TopPosition]] - `contains` [EXTRACTED]
- [[WithCloseButton]] - `contains` [EXTRACTED]
- [[close-icon.tsx]] - `imports_from` [EXTRACTED]
- [[render()_2]] - `contains` [EXTRACTED]
- [[root.tsx_8]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Dialog_Width
