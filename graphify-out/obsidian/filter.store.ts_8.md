---
source_file: "web/core/store/issue/module/filter.store.ts"
type: "code"
community: "Filters Issue"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Filters_Issue
---

# filter.store.ts

## Connections

- [[IBaseIssueFilterStore]] - `imports` [EXTRACTED]
- [[IIssueRootStore]] - `imports` [EXTRACTED]
- [[IModuleIssuesFilter]] - `contains` [EXTRACTED]
- [[IssueFilterHelperStore]] - `imports` [EXTRACTED]
- [[IssueFiltersService]] - `imports` [EXTRACTED]
- [[ModuleIssuesFilter]] - `contains` [EXTRACTED]
- [[_filters_3]] - `contains` [EXTRACTED]
- [[api.service.ts_1]] - `imports_from` [EXTRACTED]
- [[displayFilters_5]] - `contains` [EXTRACTED]
- [[displayProperties_2]] - `contains` [EXTRACTED]
- [[filterParams_3]] - `contains` [EXTRACTED]
- [[issue-filter-helper.store.ts]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_9]] - `imports_from` [EXTRACTED]
- [[issue_filter.service.ts]] - `imports_from` [EXTRACTED]
- [[kanbanFilters_3]] - `contains` [EXTRACTED]
- [[paginationParams_3]] - `contains` [EXTRACTED]
- [[root.store.ts_3]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[updatedDisplayFilters_3]] - `contains` [EXTRACTED]
- [[updatedDisplayProperties_3]] - `contains` [EXTRACTED]
- [[updatedKanbanFilters_3]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Filters_Issue
