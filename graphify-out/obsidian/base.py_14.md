---
source_file: "api/plane/app/views/cycle/base.py"
type: "code"
community: "Cycle Endpoint"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Cycle_Endpoint
---

# base.py

## Connections

- [[TODO Soft delete the cycle break the onetoone relationship with cycle issue]] - `rationale_for` [EXTRACTED]
- [[CycleAnalyticsEndpoint]] - `contains` [EXTRACTED]
- [[CycleDateCheckEndpoint]] - `contains` [EXTRACTED]
- [[CycleFavoriteViewSet]] - `contains` [EXTRACTED]
- [[CycleProgressEndpoint]] - `contains` [EXTRACTED]
- [[CycleUserPropertiesEndpoint]] - `contains` [EXTRACTED]
- [[CycleViewSet]] - `contains` [EXTRACTED]
- [[TransferCycleIssueEndpoint]] - `contains` [EXTRACTED]
- [[__init__.py_3]] - `imports_from` [EXTRACTED]
- [[create()_15]] - `contains` [EXTRACTED]
- [[destroy()_14]] - `contains` [EXTRACTED]
- [[get()_23]] - `contains` [EXTRACTED]
- [[list()_14]] - `contains` [EXTRACTED]
- [[partial_update()_11]] - `contains` [EXTRACTED]
- [[patch()_11]] - `contains` [EXTRACTED]
- [[post()_14]] - `contains` [EXTRACTED]
- [[retrieve()_9]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Cycle_Endpoint
