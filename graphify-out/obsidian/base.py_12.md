---
source_file: "api/plane/app/views/integration/base.py"
type: "code"
community: "Endpoint User"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Endpoint_User
---

# base.py

## Connections

- [[GithubAppCallbackEndpoint]] - `contains` [EXTRACTED]
- [[GithubPRStateMappingViewSet]] - `contains` [EXTRACTED]
- [[GithubRepoSyncViewSet]] - `contains` [EXTRACTED]
- [[IntegrationViewSet]] - `contains` [EXTRACTED]
- [[UserGithubConnectionView]] - `contains` [EXTRACTED]
- [[WorkspaceIntegrationViewSet]] - `contains` [EXTRACTED]
- [[__init__.py_3]] - `imports_from` [EXTRACTED]
- [[_postmessage_html()]] - `contains` [EXTRACTED]
- [[create()_14]] - `contains` [EXTRACTED]
- [[destroy()_13]] - `contains` [EXTRACTED]
- [[list()_13]] - `contains` [EXTRACTED]
- [[provider_destroy()]] - `contains` [EXTRACTED]
- [[provider_install()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Endpoint_User
