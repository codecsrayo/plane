---
source_file: "propel/src/scrollarea/scrollarea.stories.tsx"
type: "code"
community: "Community 218"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_218
---

# scrollarea.stories.tsx

## Connections
- [[BothDirections]] - `contains` [EXTRACTED]
- [[ChatMessages]] - `contains` [EXTRACTED]
- [[CodeBlock_1]] - `contains` [EXTRACTED]
- [[DataTable]] - `contains` [EXTRACTED]
- [[Default_21]] - `contains` [EXTRACTED]
- [[HorizontalScroll]] - `contains` [EXTRACTED]
- [[ListExample]] - `contains` [EXTRACTED]
- [[ScrollArea()_1]] - `imports` [EXTRACTED]
- [[ScrollTypeAlways]] - `contains` [EXTRACTED]
- [[ScrollTypeHover]] - `contains` [EXTRACTED]
- [[ScrollTypeScroll]] - `contains` [EXTRACTED]
- [[Sizes]] - `contains` [EXTRACTED]
- [[Story_24]] - `contains` [EXTRACTED]
- [[scrollarea.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_218