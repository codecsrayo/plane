---
source_file: "web/core/hooks/use-multiple-select.ts"
type: "code"
community: "Active Cycle Stats"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Active_Cycle_Stats
---

# use-multiple-select.ts

## Connections

- [[Props_1]] - `contains` [EXTRACTED]
- [[TEntityDetails]] - `contains` [EXTRACTED]
- [[TSelectionHelper]] - `contains` [EXTRACTED]
- [[TSelectionSnapshot]] - `contains` [EXTRACTED]
- [[block-root.tsx]] - `imports_from` [EXTRACTED]
- [[block-row.tsx]] - `imports_from` [EXTRACTED]
- [[block.tsx_1]] - `imports_from` [EXTRACTED]
- [[block.tsx_4]] - `imports_from` [EXTRACTED]
- [[blocks-list.tsx_1]] - `imports_from` [EXTRACTED]
- [[entity-select-action.tsx]] - `imports_from` [EXTRACTED]
- [[group-by-card.tsx_1]] - `imports_from` [EXTRACTED]
- [[group-select-action.tsx]] - `imports_from` [EXTRACTED]
- [[issue-row.tsx]] - `imports_from` [EXTRACTED]
- [[list-group.tsx]] - `imports_from` [EXTRACTED]
- [[multiple_select.store.ts]] - `imports_from` [EXTRACTED]
- [[root.tsx_6]] - `imports_from` [EXTRACTED]
- [[select-group.tsx]] - `imports_from` [EXTRACTED]
- [[sidebar.tsx_1]] - `imports_from` [EXTRACTED]
- [[spreadsheet-header.tsx]] - `imports_from` [EXTRACTED]
- [[spreadsheet-table.tsx]] - `imports_from` [EXTRACTED]
- [[use-multiple-select-store.ts]] - `imports_from` [EXTRACTED]
- [[use-reload-confirmation.tsx]] - `imports_from` [EXTRACTED]
- [[useMultipleSelect()]] - `contains` [EXTRACTED]
- [[useMultipleSelectStore()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Active_Cycle_Stats
