---
source_file: "api/plane/app/serializers/user.py"
type: "code"
community: "Community 77"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_77
---

# user.py

## Connections
- [[AccountSerializer]] - `contains` [EXTRACTED]
- [[ChangePasswordSerializer]] - `contains` [EXTRACTED]
- [[Meta_16]] - `contains` [EXTRACTED]
- [[ProfileSerializer]] - `contains` [EXTRACTED]
- [[ResetPasswordSerializer]] - `contains` [EXTRACTED]
- [[UserAdminLiteSerializer]] - `contains` [EXTRACTED]
- [[UserLiteSerializer]] - `contains` [EXTRACTED]
- [[UserMeSerializer]] - `contains` [EXTRACTED]
- [[UserMeSettingsSerializer]] - `contains` [EXTRACTED]
- [[UserSerializer]] - `contains` [EXTRACTED]
- [[__init__.py_8]] - `imports_from` [EXTRACTED]
- [[base.py_22]] - `imports_from` [EXTRACTED]
- [[exporter.py_1]] - `imports_from` [EXTRACTED]
- [[importer.py_1]] - `imports_from` [EXTRACTED]
- [[intake.py_1]] - `imports_from` [EXTRACTED]
- [[issue.py_4]] - `imports_from` [EXTRACTED]
- [[notification.py_1]] - `imports_from` [EXTRACTED]
- [[workspace.py_2]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_77