---
source_file: "store/issue/workspace-draft/issue.store.ts"
type: "code"
community: "Issue Draft"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Issue_Draft
---

# issue.store.ts

## Connections

- [[IIssueRootStore]] - `imports` [EXTRACTED]
- [[IWorkspaceDraftIssues]] - `contains` [EXTRACTED]
- [[TDraftIssuePaginationType]] - `contains` [EXTRACTED]
- [[WorkspaceDraftIssues]] - `contains` [EXTRACTED]
- [[api.service.ts]] - `imports_from` [EXTRACTED]
- [[filterParams_4]] - `contains` [EXTRACTED]
- [[issueBeforeUpdate]] - `contains` [EXTRACTED]
- [[issueIds]] - `contains` [EXTRACTED]
- [[newIssueIds]] - `contains` [EXTRACTED]
- [[params_4]] - `contains` [EXTRACTED]
- [[queryParams]] - `contains` [EXTRACTED]
- [[response]] - `contains` [EXTRACTED]
- [[root.store.ts_1]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[workspace_draft.service.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Issue_Draft
