---
source_file: "web/core/lib/store-context.tsx"
type: "code"
community: "Applied Filters UI"
location: "L14"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Applied_Filters_UI
---

# StoreContext

## Connections

- [[store-context.tsx]] - `contains` [EXTRACTED]
- [[use-analytics.ts]] - `imports` [EXTRACTED]
- [[use-app-theme.ts]] - `imports` [EXTRACTED]
- [[use-calendar-view.ts]] - `imports` [EXTRACTED]
- [[use-command-palette.ts]] - `imports` [EXTRACTED]
- [[use-cycle-filter.ts]] - `imports` [EXTRACTED]
- [[use-cycle.ts]] - `imports` [EXTRACTED]
- [[use-dashboard.ts]] - `imports` [EXTRACTED]
- [[use-editor-asset.ts]] - `imports` [EXTRACTED]
- [[use-estimate-point.ts]] - `imports` [EXTRACTED]
- [[use-estimate.ts]] - `imports` [EXTRACTED]
- [[use-favorite.ts]] - `imports` [EXTRACTED]
- [[use-global-view.ts]] - `imports` [EXTRACTED]
- [[use-home.ts]] - `imports` [EXTRACTED]
- [[use-inbox-issues.ts]] - `imports` [EXTRACTED]
- [[use-instance.ts]] - `imports` [EXTRACTED]
- [[use-issue-detail.ts]] - `imports` [EXTRACTED]
- [[use-issues.ts]] - `imports` [EXTRACTED]
- [[use-kanban-view.ts]] - `imports` [EXTRACTED]
- [[use-label.ts]] - `imports` [EXTRACTED]
- [[use-member.ts]] - `imports` [EXTRACTED]
- [[use-module-filter.ts]] - `imports` [EXTRACTED]
- [[use-module.ts]] - `imports` [EXTRACTED]
- [[use-multiple-select-store.ts]] - `imports` [EXTRACTED]
- [[use-notification.ts]] - `imports` [EXTRACTED]
- [[use-power-k.ts]] - `imports` [EXTRACTED]
- [[use-project-estimate.ts]] - `imports` [EXTRACTED]
- [[use-project-filter.ts]] - `imports` [EXTRACTED]
- [[use-project-inbox.ts]] - `imports` [EXTRACTED]
- [[use-project-publish.ts]] - `imports` [EXTRACTED]
- [[use-project-state.ts]] - `imports` [EXTRACTED]
- [[use-project-view.ts]] - `imports` [EXTRACTED]
- [[use-project.ts]] - `imports` [EXTRACTED]
- [[use-router-params.ts]] - `imports` [EXTRACTED]
- [[use-stickies.tsx]] - `imports` [EXTRACTED]
- [[use-timeline-chart.ts]] - `imports` [EXTRACTED]
- [[use-webhook.ts]] - `imports` [EXTRACTED]
- [[use-work-item-filters.ts]] - `imports` [EXTRACTED]
- [[use-workspace-draft-issue-filters.ts]] - `imports` [EXTRACTED]
- [[use-workspace-draft-issue.ts]] - `imports` [EXTRACTED]
- [[use-workspace-notifications.ts]] - `imports` [EXTRACTED]
- [[use-workspace.ts]] - `imports` [EXTRACTED]
- [[user-permissions.ts]] - `imports` [EXTRACTED]
- [[user-user-profile.ts]] - `imports` [EXTRACTED]
- [[user-user-settings.ts]] - `imports` [EXTRACTED]
- [[user-user.ts]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Applied_Filters_UI
