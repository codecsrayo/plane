---
source_file: "web/core/components/rich-filters/filters-row.tsx"
type: "code"
community: "Filter UI & Breadcrumbs"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Filter_UI__Breadcrumbs
---

# filters-row.tsx

## Connections
- [[AddFilterButton]] - `imports` [EXTRACTED]
- [[ElementTransition]] - `contains` [EXTRACTED]
- [[FilterItem]] - `imports` [EXTRACTED]
- [[FiltersRow]] - `contains` [EXTRACTED]
- [[RowTransition]] - `contains` [EXTRACTED]
- [[TAddFilterButtonProps]] - `imports` [EXTRACTED]
- [[TElementTransitionProps]] - `contains` [EXTRACTED]
- [[TFiltersRowProps]] - `contains` [EXTRACTED]
- [[TRowTransitionProps]] - `contains` [EXTRACTED]
- [[button.tsx_4]] - `imports_from` [EXTRACTED]
- [[filters-row.tsx]] - `imports_from` [EXTRACTED]
- [[root.tsx_132]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Filter_UI__Breadcrumbs