---
source_file: "web/core/components/issues/workspace-draft/draft-issue-block.tsx"
type: "code"
community: "Project Filters & Archives"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Project_Filters__Archives
---

# draft-issue-block.tsx

## Connections
- [[CreateUpdateIssueModal]] - `imports` [EXTRACTED]
- [[DraftIssueBlock]] - `contains` [EXTRACTED]
- [[DraftIssueProperties]] - `imports` [EXTRACTED]
- [[IdentifierText()]] - `imports` [EXTRACTED]
- [[IssueTypeIdentifier]] - `imports` [EXTRACTED]
- [[Props_328]] - `contains` [EXTRACTED]
- [[WorkspaceDraftIssueDeleteIssueModal()]] - `imports` [EXTRACTED]
- [[WorkspaceDraftIssueQuickActions]] - `imports` [EXTRACTED]
- [[delete-modal.tsx_1]] - `imports_from` [EXTRACTED]
- [[draft-issue-properties.tsx]] - `imports_from` [EXTRACTED]
- [[identifier-text.tsx]] - `imports_from` [EXTRACTED]
- [[index.ts_266]] - `imports_from` [EXTRACTED]
- [[modal.tsx_12]] - `imports_from` [EXTRACTED]
- [[quick-action.tsx_1]] - `imports_from` [EXTRACTED]
- [[root.tsx_70]] - `imports_from` [EXTRACTED]
- [[use-app-theme.ts]] - `imports_from` [EXTRACTED]
- [[use-project.ts]] - `imports_from` [EXTRACTED]
- [[useAppTheme()]] - `imports` [EXTRACTED]
- [[useProject()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Project_Filters__Archives