---
source_file: "web/core/components/dropdowns/buttons.tsx"
type: "code"
community: "Work Item Detail & Layouts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Work_Item_Detail__Layouts
---

# buttons.tsx

## Connections
- [[BACKGROUND_BUTTON_VARIANTS]] - `imports` [EXTRACTED]
- [[BORDER_BUTTON_VARIANTS]] - `imports` [EXTRACTED]
- [[BackgroundButton()_1]] - `contains` [EXTRACTED]
- [[BorderButton()_1]] - `contains` [EXTRACTED]
- [[ButtonProps_3]] - `contains` [EXTRACTED]
- [[DropdownButton()_1]] - `contains` [EXTRACTED]
- [[DropdownButtonProps]] - `contains` [EXTRACTED]
- [[TransparentButton()_1]] - `contains` [EXTRACTED]
- [[base.tsx_2]] - `imports_from` [EXTRACTED]
- [[base.tsx_3]] - `imports_from` [EXTRACTED]
- [[base.tsx_4]] - `imports_from` [EXTRACTED]
- [[base.tsx_5]] - `imports_from` [EXTRACTED]
- [[base.tsx_6]] - `imports_from` [EXTRACTED]
- [[constants.ts_5]] - `imports_from` [EXTRACTED]
- [[date-range.tsx]] - `imports_from` [EXTRACTED]
- [[date.tsx_1]] - `imports_from` [EXTRACTED]
- [[estimate.tsx_1]] - `imports_from` [EXTRACTED]
- [[index.tsx_12]] - `imports_from` [EXTRACTED]
- [[use-platform-os.tsx_1]] - `imports_from` [EXTRACTED]
- [[usePlatformOS()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Work_Item_Detail__Layouts