---
source_file: "editor/src/core/helpers/common.ts"
type: "code"
community: "Community 30"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_30
---

# common.ts

## Connections
- [[CORE_EXTENSIONS]] - `imports` [EXTRACTED]
- [[EditorClassNameArgs]] - `contains` [EXTRACTED]
- [[collaborative-editor.tsx]] - `imports_from` [EXTRACTED]
- [[editor-commands.ts]] - `imports_from` [EXTRACTED]
- [[editor-ref.ts]] - `imports_from` [EXTRACTED]
- [[editor-wrapper.tsx]] - `imports_from` [EXTRACTED]
- [[editor.tsx_1]] - `imports_from` [EXTRACTED]
- [[extension.ts]] - `imports_from` [EXTRACTED]
- [[extension.tsx]] - `imports_from` [EXTRACTED]
- [[findParentNodeOfType()]] - `contains` [EXTRACTED]
- [[findTableAncestor()]] - `contains` [EXTRACTED]
- [[getEditorClassNames()]] - `contains` [EXTRACTED]
- [[getParagraphCount()]] - `contains` [EXTRACTED]
- [[getTrimmedHTML()]] - `contains` [EXTRACTED]
- [[insert-line-above-table-action.ts]] - `imports_from` [EXTRACTED]
- [[insert-line-below-table-action.ts]] - `imports_from` [EXTRACTED]
- [[isValidHttpUrl()]] - `contains` [EXTRACTED]
- [[link-edit-view.tsx]] - `imports_from` [EXTRACTED]
- [[link-selector.tsx]] - `imports_from` [EXTRACTED]
- [[table-cell.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_30