---
source_file: "web/core/components/work-item-filters/filters-hoc/project-level.tsx"
type: "code"
community: "Project Work"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Project_Work
---

# project-level.tsx

## Connections

- [[CreateUpdateProjectViewModal]] - `imports` [EXTRACTED]
- [[ProjectLevelWorkItemFiltersHOC]] - `contains` [EXTRACTED]
- [[TEnableSaveViewProps]] - `imports` [EXTRACTED]
- [[TEnableUpdateViewProps]] - `imports` [EXTRACTED]
- [[TProjectLevelWorkItemFiltersHOCProps]] - `contains` [EXTRACTED]
- [[TSharedWorkItemFiltersHOCProps]] - `imports` [EXTRACTED]
- [[WorkItemFiltersHOC]] - `imports` [EXTRACTED]
- [[archived-issue-layout-root.tsx]] - `imports_from` [EXTRACTED]
- [[base.tsx_9]] - `imports_from` [EXTRACTED]
- [[cycle-layout-root.tsx]] - `imports_from` [EXTRACTED]
- [[form.tsx]] - `imports_from` [EXTRACTED]
- [[index.ts_265]] - `imports_from` [EXTRACTED]
- [[modal.tsx_5]] - `imports_from` [EXTRACTED]
- [[module-layout-root.tsx]] - `imports_from` [EXTRACTED]
- [[project-layout-root.tsx]] - `imports_from` [EXTRACTED]
- [[project-view-layout-root.tsx]] - `imports_from` [EXTRACTED]
- [[removeNillKeys()]] - `imports` [EXTRACTED]
- [[shared.ts_13]] - `imports_from` [EXTRACTED]
- [[use-cycle.ts]] - `imports_from` [EXTRACTED]
- [[use-label.ts]] - `imports_from` [EXTRACTED]
- [[use-member.ts]] - `imports_from` [EXTRACTED]
- [[use-module.ts]] - `imports_from` [EXTRACTED]
- [[use-project-state.ts]] - `imports_from` [EXTRACTED]
- [[use-project-view.ts]] - `imports_from` [EXTRACTED]
- [[use-project.ts]] - `imports_from` [EXTRACTED]
- [[useCycle()]] - `imports` [EXTRACTED]
- [[useLabel()]] - `imports` [EXTRACTED]
- [[useMember()]] - `imports` [EXTRACTED]
- [[useModule()]] - `imports` [EXTRACTED]
- [[useProject()]] - `imports` [EXTRACTED]
- [[useProjectState()]] - `imports` [EXTRACTED]
- [[useProjectView()]] - `imports` [EXTRACTED]
- [[utils.tsx_2]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Project_Work
