---
source_file: "web/core/components/navigation/tab-navigation-utils.ts"
type: "code"
community: "Community 73"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_73
---

# tab-navigation-utils.ts

## Connections
- [[TTabPreferences]] - `contains` [EXTRACTED]
- [[getDefaultTabUrl()]] - `contains` [EXTRACTED]
- [[getTabPreferences()]] - `contains` [EXTRACTED]
- [[getTabUrl()]] - `contains` [EXTRACTED]
- [[getValidatedDefaultTab()]] - `contains` [EXTRACTED]
- [[project-header.tsx]] - `imports_from` [EXTRACTED]
- [[projects-list-item.tsx]] - `imports_from` [EXTRACTED]
- [[saveTabPreferences()]] - `contains` [EXTRACTED]
- [[tab-navigation-overflow-menu.tsx]] - `imports_from` [EXTRACTED]
- [[tab-navigation-root.tsx]] - `imports_from` [EXTRACTED]
- [[tab-navigation-visible-item.tsx]] - `imports_from` [EXTRACTED]
- [[use-tab-preferences.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_73