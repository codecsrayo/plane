---
source_file: "editor/src/core/extensions/table/table/utilities/delete-key-shortcut.ts"
type: "code"
community: "Community 150"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_150
---

# delete-key-shortcut.ts

## Connections
- [[CORE_EXTENSIONS]] - `imports` [EXTRACTED]
- [[CellCoord]] - `contains` [EXTRACTED]
- [[TableInfo]] - `contains` [EXTRACTED]
- [[calculateSelectionBounds()]] - `contains` [EXTRACTED]
- [[checkCellsHaveContent()]] - `contains` [EXTRACTED]
- [[deleteMultipleColumns()]] - `contains` [EXTRACTED]
- [[deleteMultipleRows()]] - `contains` [EXTRACTED]
- [[extension.ts]] - `imports_from` [EXTRACTED]
- [[findCellCoordinate()]] - `contains` [EXTRACTED]
- [[getSelectedCellCoords()]] - `contains` [EXTRACTED]
- [[getTableInfo()]] - `contains` [EXTRACTED]
- [[handleDeleteKeyOnTable()]] - `contains` [EXTRACTED]
- [[helpers.ts]] - `imports_from` [EXTRACTED]
- [[isCellEmpty()]] - `imports` [EXTRACTED]
- [[isCellSelection()]] - `imports` [EXTRACTED]
- [[setCursorAtPosition()]] - `contains` [EXTRACTED]
- [[table.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_150