---
source_file: "propel/src/combobox/combobox.stories.tsx"
type: "code"
community: "Community 226"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_226
---

# combobox.stories.tsx

## Connections
- [[Combobox]] - `imports` [EXTRACTED]
- [[CustomEmptyMessage]] - `contains` [EXTRACTED]
- [[CustomMaxHeight]] - `contains` [EXTRACTED]
- [[Default_11]] - `contains` [EXTRACTED]
- [[Disabled_1]] - `contains` [EXTRACTED]
- [[DisabledOptions]] - `contains` [EXTRACTED]
- [[MultiSelect]] - `contains` [EXTRACTED]
- [[MultiSelectWithLimit]] - `contains` [EXTRACTED]
- [[Story_12]] - `contains` [EXTRACTED]
- [[WithoutSearch]] - `contains` [EXTRACTED]
- [[combobox.tsx]] - `imports_from` [EXTRACTED]
- [[frameworks]] - `contains` [EXTRACTED]
- [[index.ts_138]] - `imports_from` [EXTRACTED]
- [[render()_1]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_226