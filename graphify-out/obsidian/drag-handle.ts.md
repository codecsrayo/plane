---
source_file: "editor/src/core/plugins/drag-handle.ts"
type: "code"
community: "Handle Node"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Handle_Node
---

# drag-handle.ts

## Connections

- [[CORE_EXTENSIONS]] - `imports` [EXTRACTED]
- [[DragHandlePlugin()]] - `contains` [EXTRACTED]
- [[ai-handle.ts]] - `imports_from` [EXTRACTED]
- [[createDragHandleElement()]] - `contains` [EXTRACTED]
- [[easeOutQuadAnimation()]] - `contains` [EXTRACTED]
- [[extension.ts]] - `imports_from` [EXTRACTED]
- [[flattenListStructure()]] - `contains` [EXTRACTED]
- [[generalSelectors]] - `contains` [EXTRACTED]
- [[getScrollParent()]] - `contains` [EXTRACTED]
- [[handleNodeSelection()]] - `contains` [EXTRACTED]
- [[index.ts_47]] - `imports_from` [EXTRACTED]
- [[isScrollable()]] - `contains` [EXTRACTED]
- [[nodeDOMAtCoords()]] - `contains` [EXTRACTED]
- [[nodePosAtDOM()_1]] - `contains` [EXTRACTED]
- [[nodePosAtDOMForBlockQuotes()_1]] - `contains` [EXTRACTED]
- [[scrollParentCache]] - `contains` [EXTRACTED]
- [[side-menu.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Handle_Node
