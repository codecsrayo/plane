---
source_file: "space/hooks/store/use-instance.ts"
type: "code"
community: "Workspace Issue"
location: "L13"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Workspace_Issue
---

# useInstance()

## Connections

- [[auth-root.tsx]] - `imports` [EXTRACTED]
- [[auth-root.tsx_1]] - `imports` [EXTRACTED]
- [[changelog.tsx_1]] - `imports` [EXTRACTED]
- [[command.ts]] - `imports` [EXTRACTED]
- [[core.tsx_1]] - `imports` [EXTRACTED]
- [[core.tsx_4]] - `imports` [EXTRACTED]
- [[create-or-join-workspaces.tsx]] - `imports` [EXTRACTED]
- [[create.tsx_1]] - `imports` [EXTRACTED]
- [[description-editor.tsx]] - `imports` [EXTRACTED]
- [[form-root.tsx]] - `imports` [EXTRACTED]
- [[form.tsx_5]] - `imports` [EXTRACTED]
- [[header.tsx_56]] - `imports` [EXTRACTED]
- [[image-picker-popover.tsx]] - `imports` [EXTRACTED]
- [[instance-provider.tsx]] - `imports` [EXTRACTED]
- [[instance-wrapper.tsx]] - `imports` [EXTRACTED]
- [[page.tsx_52]] - `imports` [EXTRACTED]
- [[page.tsx_53]] - `imports` [EXTRACTED]
- [[page.tsx_64]] - `imports` [EXTRACTED]
- [[root.tsx_109]] - `imports` [EXTRACTED]
- [[select-channel.tsx]] - `imports` [EXTRACTED]
- [[single-integration-card.tsx]] - `imports` [EXTRACTED]
- [[use-chat-support.ts]] - `imports` [EXTRACTED]
- [[use-file-size.ts]] - `imports` [EXTRACTED]
- [[use-instance.ts]] - `contains` [EXTRACTED]
- [[use-instance.ts_1]] - `contains` [EXTRACTED]
- [[use-instance.tsx]] - `contains` [EXTRACTED]
- [[workspace-menu-root.tsx]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Workspace_Issue
