---
source_file: "web/core/components/pages/dropdowns/actions.tsx"
type: "code"
community: "Community 456"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_456
---

# actions.tsx

## Connections
- [[DeletePageModal]] - `imports` [EXTRACTED]
- [[PageActions]] - `contains` [EXTRACTED]
- [[Props_481]] - `contains` [EXTRACTED]
- [[TPageActions]] - `contains` [EXTRACTED]
- [[TPageInstance]] - `imports` [EXTRACTED]
- [[base-page.ts]] - `imports_from` [EXTRACTED]
- [[delete-page-modal.tsx]] - `imports_from` [EXTRACTED]
- [[use-page-operations.ts]] - `imports_from` [EXTRACTED]
- [[usePageFlag()]] - `imports` [EXTRACTED]
- [[usePageOperations()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_456