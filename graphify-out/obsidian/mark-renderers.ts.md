---
source_file: "live/src/lib/pdf/mark-renderers.ts"
type: "code"
community: "Community 55"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_55
---

# mark-renderers.ts

## Connections
- [[BACKGROUND_COLORS]] - `imports` [EXTRACTED]
- [[CODE_COLORS]] - `imports` [EXTRACTED]
- [[EDITOR_TEXT_COLORS]] - `imports` [EXTRACTED]
- [[LINK_COLORS]] - `imports` [EXTRACTED]
- [[MarkRendererRegistry]] - `imports` [EXTRACTED]
- [[TipTapMark]] - `imports` [EXTRACTED]
- [[applyMarks()]] - `contains` [EXTRACTED]
- [[colors.ts]] - `imports_from` [EXTRACTED]
- [[markRenderers]] - `contains` [EXTRACTED]
- [[node-renderers.tsx]] - `imports_from` [EXTRACTED]
- [[resolveColorForPdf()]] - `imports` [EXTRACTED]
- [[types.ts_20]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_55