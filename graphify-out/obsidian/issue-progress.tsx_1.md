---
source_file: "web/core/components/cycles/analytics-sidebar/issue-progress.tsx"
type: "code"
community: "Active Cycle Stats"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Active_Cycle_Stats
---

# issue-progress.tsx

## Connections
- [[CycleAnalyticsProgress]] - `contains` [EXTRACTED]
- [[CycleProgressStats]] - `imports` [EXTRACTED]
- [[Options]] - `contains` [EXTRACTED]
- [[TCycleAnalyticsProgress]] - `contains` [EXTRACTED]
- [[base.tsx]] - `imports_from` [EXTRACTED]
- [[cycleChartOptions]] - `contains` [EXTRACTED]
- [[cycleEstimateOptions]] - `contains` [EXTRACTED]
- [[estimate-type-dropdown.tsx]] - `imports_from` [EXTRACTED]
- [[progress-stats.tsx_1]] - `imports_from` [EXTRACTED]
- [[root.tsx_94]] - `imports_from` [EXTRACTED]
- [[use-cycle.ts]] - `imports_from` [EXTRACTED]
- [[use-work-item-filters.ts]] - `imports_from` [EXTRACTED]
- [[useCycle()]] - `imports` [EXTRACTED]
- [[useWorkItemFilters()]] - `imports` [EXTRACTED]
- [[validateCycleSnapshot()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Active_Cycle_Stats