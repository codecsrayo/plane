---
source_file: "propel/src/design-system/design-system-philosophy.stories.tsx"
type: "code"
community: "Community 305"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_305
---

# design-system-philosophy.stories.tsx

## Connections
- [[ApplicationRoot]] - `contains` [EXTRACTED]
- [[CardListPattern]] - `contains` [EXTRACTED]
- [[CommonMistakes]] - `contains` [EXTRACTED]
- [[CompleteExample]] - `contains` [EXTRACTED]
- [[ContainerProps]] - `contains` [EXTRACTED]
- [[DemoRoot()]] - `contains` [EXTRACTED]
- [[Info()]] - `contains` [EXTRACTED]
- [[Layer()]] - `contains` [EXTRACTED]
- [[LayerStacking]] - `contains` [EXTRACTED]
- [[ModalException]] - `contains` [EXTRACTED]
- [[SidebarLayoutPattern]] - `contains` [EXTRACTED]
- [[StateVariants]] - `contains` [EXTRACTED]
- [[Story_39]] - `contains` [EXTRACTED]
- [[Surface()]] - `contains` [EXTRACTED]
- [[SurfaceLayerAssociation]] - `contains` [EXTRACTED]
- [[SurfaceSiblings]] - `contains` [EXTRACTED]
- [[TextColorHierarchy]] - `contains` [EXTRACTED]
- [[TwoColGrid()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_305