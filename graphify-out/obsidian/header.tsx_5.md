---
source_file: "web/app/(all)/[workspaceSlug]/(projects)/projects/(detail)/[projectId]/views/(detail)/[viewId]/header.tsx"
type: "code"
community: "Filter UI & Breadcrumbs"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Filter_UI__Breadcrumbs
---

# header.tsx

## Connections
- [[BreadcrumbLink()]] - `imports` [EXTRACTED]
- [[CommonProjectBreadcrumbs()]] - `imports` [EXTRACTED]
- [[ProjectViewIssuesHeader]] - `contains` [EXTRACTED]
- [[SwitcherIcon()]] - `imports` [EXTRACTED]
- [[SwitcherLabel()]] - `imports` [EXTRACTED]
- [[ViewQuickActions]] - `imports` [EXTRACTED]
- [[WorkItemFiltersToggle]] - `imports` [EXTRACTED]
- [[breadcrumb-link.tsx]] - `imports_from` [EXTRACTED]
- [[filters-toggle.tsx]] - `imports_from` [EXTRACTED]
- [[index.ts_355]] - `imports_from` [EXTRACTED]
- [[index.ts_265]] - `imports_from` [EXTRACTED]
- [[layout.tsx_11]] - `imports_from` [EXTRACTED]
- [[quick-actions.tsx]] - `imports_from` [EXTRACTED]
- [[switcher-label.tsx]] - `imports_from` [EXTRACTED]
- [[use-app-router.tsx]] - `imports_from` [EXTRACTED]
- [[use-command-palette.ts]] - `imports_from` [EXTRACTED]
- [[use-issues.ts]] - `imports_from` [EXTRACTED]
- [[use-project-view.ts]] - `imports_from` [EXTRACTED]
- [[use-project.ts]] - `imports_from` [EXTRACTED]
- [[useAppRouter()]] - `imports` [EXTRACTED]
- [[useCommandPalette()]] - `imports` [EXTRACTED]
- [[useIssues()]] - `imports` [EXTRACTED]
- [[useProject()]] - `imports` [EXTRACTED]
- [[useProjectView()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Filter_UI__Breadcrumbs