---
source_file: "web/core/store/pages/project-page.store.ts"
type: "code"
community: "Community 136"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_136
---

# project-page.store.ts

## Connections
- [[CoreRootStore]] - `imports` [EXTRACTED]
- [[IProjectPageStore]] - `contains` [EXTRACTED]
- [[ProjectPage]] - `imports` [EXTRACTED]
- [[ProjectPageStore]] - `contains` [EXTRACTED]
- [[ROLE_PERMISSIONS_TO_CREATE_PAGE]] - `contains` [EXTRACTED]
- [[TError_3]] - `contains` [EXTRACTED]
- [[TLoader_2]] - `contains` [EXTRACTED]
- [[TProjectPage]] - `imports` [EXTRACTED]
- [[api.service.ts_1]] - `imports_from` [EXTRACTED]
- [[currentPageId]] - `contains` [EXTRACTED]
- [[existingPage]] - `contains` [EXTRACTED]
- [[filteredPages]] - `contains` [EXTRACTED]
- [[index.ts_285]] - `imports_from` [EXTRACTED]
- [[pageInstance]] - `contains` [EXTRACTED]
- [[pages]] - `contains` [EXTRACTED]
- [[pagesByType]] - `contains` [EXTRACTED]
- [[project-page.ts]] - `imports_from` [EXTRACTED]
- [[root.store.ts_2]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[use-page-store.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_136