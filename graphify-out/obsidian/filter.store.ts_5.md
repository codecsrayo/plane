---
source_file: "web/core/store/issue/profile/filter.store.ts"
type: "code"
community: "Community 118"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_118
---

# filter.store.ts

## Connections

- [[IBaseIssueFilterStore]] - `imports` [EXTRACTED]
- [[IIssueRootStore]] - `imports` [EXTRACTED]
- [[IProfileIssuesFilter]] - `contains` [EXTRACTED]
- [[IssueFilterHelperStore]] - `imports` [EXTRACTED]
- [[IssueFiltersService]] - `imports` [EXTRACTED]
- [[ProfileIssuesFilter]] - `contains` [EXTRACTED]
- [[_filters_5]] - `contains` [EXTRACTED]
- [[api.service.ts]] - `imports_from` [EXTRACTED]
- [[displayFilters_7]] - `contains` [EXTRACTED]
- [[displayProperties_4]] - `contains` [EXTRACTED]
- [[filterParams_6]] - `contains` [EXTRACTED]
- [[issue-filter-helper.store.ts]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_6]] - `imports_from` [EXTRACTED]
- [[issue_filter.service.ts]] - `imports_from` [EXTRACTED]
- [[kanbanFilters_5]] - `contains` [EXTRACTED]
- [[paginationParams_5]] - `contains` [EXTRACTED]
- [[root.store.ts_1]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[updatedDisplayFilters_5]] - `contains` [EXTRACTED]
- [[updatedDisplayProperties_5]] - `contains` [EXTRACTED]
- [[updatedKanbanFilters_4]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_118
