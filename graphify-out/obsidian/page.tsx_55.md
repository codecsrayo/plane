---
source_file: "web/app/(all)/[workspaceSlug]/(settings)/settings/(workspace)/webhooks/[webhookId]/page.tsx"
type: "code"
community: "Webhook Workspace"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Webhook_Workspace
---

# page.tsx

## Connections

- [[LogoSpinner()]] - `imports` [EXTRACTED]
- [[PageHead()]] - `imports` [EXTRACTED]
- [[SettingsContentWrapper()]] - `imports` [EXTRACTED]
- [[WebhookDetailsPage()]] - `contains` [EXTRACTED]
- [[WebhookDetailsWorkspaceSettingsHeader]] - `imports` [EXTRACTED]
- [[content-wrapper.tsx_2]] - `imports_from` [EXTRACTED]
- [[header.tsx_36]] - `imports_from` [EXTRACTED]
- [[index.ts_347]] - `imports_from` [EXTRACTED]
- [[index.ts_265]] - `imports_from` [EXTRACTED]
- [[logo-spinner.tsx]] - `imports_from` [EXTRACTED]
- [[page-title.tsx]] - `imports_from` [EXTRACTED]
- [[use-webhook.ts]] - `imports_from` [EXTRACTED]
- [[use-workspace.ts]] - `imports_from` [EXTRACTED]
- [[useWebhook()]] - `imports` [EXTRACTED]
- [[useWorkspace()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Webhook_Workspace
