---
source_file: "api_rust/src/utils/pagination.rs"
type: "code"
community: "Community 156"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_156
---

# pagination.rs

## Connections

- [[Cursor]] - `contains` [EXTRACTED]
- [[build_response()]] - `contains` [EXTRACTED]
- [[default_when_empty()]] - `contains` [EXTRACTED]
- [[parse_cursor_or_default()]] - `contains` [EXTRACTED]
- [[parse_standard_cursor()]] - `contains` [EXTRACTED]
- [[parse_with_is_prev()]] - `contains` [EXTRACTED]
- [[reject_malformed_cursor()]] - `contains` [EXTRACTED]
- [[resolve_per_page()]] - `contains` [EXTRACTED]
- [[response_shape()]] - `contains` [EXTRACTED]
- [[response_shape_last_page()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_156
