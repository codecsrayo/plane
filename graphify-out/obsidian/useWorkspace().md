---
source_file: "admin/hooks/store/use-workspace.tsx"
type: "code"
community: "Community 65"
location: "L12"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_65
---

# useWorkspace()

## Connections
- [[activity-list.tsx]] - `imports` [EXTRACTED]
- [[ask-pi-menu.tsx]] - `imports` [EXTRACTED]
- [[authentication-wrapper.tsx]] - `imports` [EXTRACTED]
- [[comment-create.tsx]] - `imports` [EXTRACTED]
- [[core.tsx_1]] - `imports` [EXTRACTED]
- [[create-root.tsx]] - `imports` [EXTRACTED]
- [[create-workspace-form.tsx]] - `imports` [EXTRACTED]
- [[create-workspace.tsx]] - `imports` [EXTRACTED]
- [[create.tsx_1]] - `imports` [EXTRACTED]
- [[delete-workspace-form.tsx]] - `imports` [EXTRACTED]
- [[description-editor.tsx]] - `imports` [EXTRACTED]
- [[editor-body.tsx]] - `imports` [EXTRACTED]
- [[editor.tsx_7]] - `imports` [EXTRACTED]
- [[form-root.tsx]] - `imports` [EXTRACTED]
- [[header.tsx_45]] - `imports` [EXTRACTED]
- [[header.tsx_47]] - `imports` [EXTRACTED]
- [[inputs.tsx]] - `imports` [EXTRACTED]
- [[invitations.tsx]] - `imports` [EXTRACTED]
- [[item.tsx_2]] - `imports` [EXTRACTED]
- [[join-invites.tsx]] - `imports` [EXTRACTED]
- [[members-list-item.tsx]] - `imports` [EXTRACTED]
- [[modal.tsx_19]] - `imports` [EXTRACTED]
- [[no-projects.tsx]] - `imports` [EXTRACTED]
- [[page.tsx_7]] - `imports` [EXTRACTED]
- [[page.tsx_13]] - `imports` [EXTRACTED]
- [[page.tsx_27]] - `imports` [EXTRACTED]
- [[page.tsx_31]] - `imports` [EXTRACTED]
- [[page.tsx_32]] - `imports` [EXTRACTED]
- [[page.tsx_33]] - `imports` [EXTRACTED]
- [[page.tsx_34]] - `imports` [EXTRACTED]
- [[page.tsx_48]] - `imports` [EXTRACTED]
- [[page.tsx_49]] - `imports` [EXTRACTED]
- [[page.tsx_50]] - `imports` [EXTRACTED]
- [[page.tsx_51]] - `imports` [EXTRACTED]
- [[page.tsx_52]] - `imports` [EXTRACTED]
- [[page.tsx_53]] - `imports` [EXTRACTED]
- [[page.tsx_54]] - `imports` [EXTRACTED]
- [[page.tsx_55]] - `imports` [EXTRACTED]
- [[page.tsx_58]] - `imports` [EXTRACTED]
- [[page.tsx_62]] - `imports` [EXTRACTED]
- [[page.tsx_65]] - `imports` [EXTRACTED]
- [[root.tsx_51]] - `imports` [EXTRACTED]
- [[root.tsx_65]] - `imports` [EXTRACTED]
- [[root.tsx_85]] - `imports` [EXTRACTED]
- [[root.tsx_86]] - `imports` [EXTRACTED]
- [[root.tsx_104]] - `imports` [EXTRACTED]
- [[root.tsx_108]] - `imports` [EXTRACTED]
- [[use-navigation-preferences.ts]] - `imports` [EXTRACTED]
- [[use-workspace.ts]] - `contains` [EXTRACTED]
- [[use-workspace.tsx]] - `contains` [EXTRACTED]
- [[user.tsx_1]] - `imports` [EXTRACTED]
- [[workspace-details.tsx]] - `imports` [EXTRACTED]
- [[workspace-image-upload-modal.tsx]] - `imports` [EXTRACTED]
- [[workspace-menu-root.tsx]] - `imports` [EXTRACTED]
- [[workspace-options.tsx]] - `imports` [EXTRACTED]
- [[workspace-wrapper.tsx]] - `imports` [EXTRACTED]
- [[workspaces-menu.tsx]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_65