---
source_file: "web/core/store/inbox/project-inbox.store.ts"
type: "code"
community: "Project Issue"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Project_Issue
---

# project-inbox.store.ts

## Connections

- [[CoreRootStore]] - `imports` [EXTRACTED]
- [[IInboxIssueStore]] - `imports` [EXTRACTED]
- [[IProjectInboxStore]] - `contains` [EXTRACTED]
- [[InboxIssueStore]] - `imports` [EXTRACTED]
- [[ProjectInboxStore]] - `contains` [EXTRACTED]
- [[TLoader_1]] - `contains` [EXTRACTED]
- [[api.service.ts_1]] - `imports_from` [EXTRACTED]
- [[appliedDateFilters]] - `contains` [EXTRACTED]
- [[dateValue]] - `contains` [EXTRACTED]
- [[existingInboxIssueDetail]] - `contains` [EXTRACTED]
- [[filterKey_1]] - `contains` [EXTRACTED]
- [[filters_2]] - `contains` [EXTRACTED]
- [[inbox-issue.store.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_288]] - `imports_from` [EXTRACTED]
- [[issueIds_1]] - `contains` [EXTRACTED]
- [[queryParams_2]] - `contains` [EXTRACTED]
- [[root.store.ts_2]] - `imports_from` [EXTRACTED]
- [[sorting]] - `contains` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Project_Issue
