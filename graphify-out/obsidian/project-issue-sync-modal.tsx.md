---
source_file: "web/core/components/integration/github/project-issue-sync-modal.tsx"
type: "code"
community: "Community 101"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_101
---

# project-issue-sync-modal.tsx

## Connections

- [[GithubProjectIssueSyncModal]] - `contains` [EXTRACTED]
- [[INITIAL_STATE]] - `contains` [EXTRACTED]
- [[IntegrationError]] - `contains` [EXTRACTED]
- [[ModalAction]] - `contains` [EXTRACTED]
- [[ModalState]] - `contains` [EXTRACTED]
- [[Props_156]] - `contains` [EXTRACTED]
- [[SyncDirection]] - `contains` [EXTRACTED]
- [[getGithubReposSwrKey()]] - `imports` [EXTRACTED]
- [[index.ts_21]] - `imports_from` [EXTRACTED]
- [[modalReducer()]] - `contains` [EXTRACTED]
- [[use-project-state.ts]] - `imports_from` [EXTRACTED]
- [[use-project.ts]] - `imports_from` [EXTRACTED]
- [[useProject()]] - `imports` [EXTRACTED]
- [[useProjectState()]] - `imports` [EXTRACTED]
- [[utils.ts_3]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_101
