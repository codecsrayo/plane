---
source_file: "utils/src/theme/color-conversion.ts"
type: "code"
community: "Community 87"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_87
---

# color-conversion.ts

## Connections
- [[OKLCH]] - `contains` [EXTRACTED]
- [[RGB]] - `contains` [EXTRACTED]
- [[color-validation.ts]] - `imports_from` [EXTRACTED]
- [[getPerceptualBrightness()]] - `contains` [EXTRACTED]
- [[getRelativeLuminance()]] - `contains` [EXTRACTED]
- [[hexToHSL()]] - `contains` [EXTRACTED]
- [[hexToOKLCH()]] - `contains` [EXTRACTED]
- [[hexToOKLCHString()]] - `contains` [EXTRACTED]
- [[hexToRgb()_1]] - `contains` [EXTRACTED]
- [[isGrayscale()]] - `contains` [EXTRACTED]
- [[oklchToCSS()]] - `contains` [EXTRACTED]
- [[palette-generator.ts]] - `imports_from` [EXTRACTED]
- [[parseOKLCH()]] - `contains` [EXTRACTED]
- [[rgbToHex()_1]] - `contains` [EXTRACTED]
- [[theme-application.ts]] - `imports_from` [EXTRACTED]
- [[toHex()_1]] - `contains` [EXTRACTED]
- [[validateAndAdjustOKLCH()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_87