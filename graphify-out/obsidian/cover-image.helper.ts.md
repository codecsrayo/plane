---
source_file: "web/helpers/cover-image.helper.ts"
type: "code"
community: "Cover Image"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Cover_Image
---

# cover-image.helper.ts

## Connections

- [[FileService_1]] - `imports` [EXTRACTED]
- [[STATIC_COVER_IMAGES]] - `contains` [EXTRACTED]
- [[STATIC_COVER_IMAGES_SET]] - `contains` [EXTRACTED]
- [[TCoverImagePayload]] - `contains` [EXTRACTED]
- [[TCoverImageResult]] - `contains` [EXTRACTED]
- [[TCoverImageType]] - `contains` [EXTRACTED]
- [[analyzeCoverImageChange()]] - `contains` [EXTRACTED]
- [[file.service.ts_1]] - `imports_from` [EXTRACTED]
- [[fileService]] - `contains` [EXTRACTED]
- [[getCoverImageDisplayURL()]] - `contains` [EXTRACTED]
- [[getCoverImageType()]] - `contains` [EXTRACTED]
- [[getRandomCoverImage()]] - `contains` [EXTRACTED]
- [[handleCoverImageChange()]] - `contains` [EXTRACTED]
- [[isStaticCoverImage()]] - `contains` [EXTRACTED]
- [[uploadCoverImage()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Cover_Image
