---
source_file: "web/core/services/file.service.ts"
type: "code"
community: "Issue Project"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Issue_Project
---

# file.service.ts

## Connections

- [[ApiError]] - `imports` [EXTRACTED]
- [[FileService_1]] - `contains` [EXTRACTED]
- [[FileUploadService_1]] - `imports` [EXTRACTED]
- [[TFileAssetType]] - `contains` [EXTRACTED]
- [[UnSplashImage]] - `contains` [EXTRACTED]
- [[UnSplashImageUrls]] - `contains` [EXTRACTED]
- [[api.service.ts_1]] - `imports_from` [EXTRACTED]
- [[asset.store.ts]] - `imports_from` [EXTRACTED]
- [[base.tsx_8]] - `imports_from` [EXTRACTED]
- [[comment-create.tsx]] - `imports_from` [EXTRACTED]
- [[cover-image.helper.ts]] - `imports_from` [EXTRACTED]
- [[create-project-modal.tsx]] - `imports_from` [EXTRACTED]
- [[create-root.tsx]] - `imports_from` [EXTRACTED]
- [[file-upload.service.ts_1]] - `imports_from` [EXTRACTED]
- [[image-picker-popover.tsx]] - `imports_from` [EXTRACTED]
- [[use-editor-config.ts]] - `imports_from` [EXTRACTED]
- [[user-image-upload-modal.tsx]] - `imports_from` [EXTRACTED]
- [[workspace-image-upload-modal.tsx]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Issue_Project
