---
source_file: "web/core/store/member/project/base-project-member.store.ts"
type: "code"
community: "Community 173"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_173
---

# base-project-member.store.ts

## Connections

- [[IBaseProjectMemberStore]] - `contains` [EXTRACTED]
- [[IMemberRootStore]] - `imports` [EXTRACTED]
- [[IProjectMemberDetails]] - `contains` [EXTRACTED]
- [[IProjectMemberFiltersStore]] - `imports` [EXTRACTED]
- [[IProjectStore]] - `imports` [EXTRACTED]
- [[IRouterStore]] - `imports` [EXTRACTED]
- [[IUserStore]] - `imports` [EXTRACTED]
- [[ProjectMemberFiltersStore]] - `imports` [EXTRACTED]
- [[allMembers]] - `contains` [EXTRACTED]
- [[api.service.ts]] - `imports_from` [EXTRACTED]
- [[constructor()_7]] - `contains` [EXTRACTED]
- [[filteredMemberIds_1]] - `contains` [EXTRACTED]
- [[index.ts_22]] - `imports_from` [EXTRACTED]
- [[index.ts_7]] - `imports_from` [EXTRACTED]
- [[index.ts_17]] - `imports_from` [EXTRACTED]
- [[member-list-item.tsx]] - `imports_from` [EXTRACTED]
- [[memberDetails_1]] - `contains` [EXTRACTED]
- [[memberIds_1]] - `contains` [EXTRACTED]
- [[members_1]] - `contains` [EXTRACTED]
- [[membershipBeforeUpdate]] - `contains` [EXTRACTED]
- [[project-member-filters.store.ts]] - `imports_from` [EXTRACTED]
- [[project.store.ts]] - `imports_from` [EXTRACTED]
- [[projectMember]] - `contains` [EXTRACTED]
- [[projectMemberIds()]] - `contains` [EXTRACTED]
- [[projectMembership]] - `contains` [EXTRACTED]
- [[router.store.ts]] - `imports_from` [EXTRACTED]
- [[sortProjectMembers()]] - `imports` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[updatedProjectRole]] - `contains` [EXTRACTED]
- [[utils.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_173
