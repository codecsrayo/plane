---
source_file: "web/app/(all)/[workspaceSlug]/(projects)/_sidebar.tsx"
type: "code"
community: "Sidebar Resizable"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Sidebar_Resizable
---

# \_sidebar.tsx

## Connections

- [[AppSidebar]] - `imports` [EXTRACTED]
- [[ExtendedAppSidebar]] - `imports` [EXTRACTED]
- [[ProjectAppSidebar]] - `contains` [EXTRACTED]
- [[ResizableSidebar()]] - `imports` [EXTRACTED]
- [[SidebarFavoritesMenu]] - `imports` [EXTRACTED]
- [[SidebarMenuItems]] - `imports` [EXTRACTED]
- [[SidebarProjectsList]] - `imports` [EXTRACTED]
- [[SidebarQuickActions]] - `imports` [EXTRACTED]
- [[SidebarTeamsList()]] - `imports` [EXTRACTED]
- [[SidebarWrapper]] - `imports` [EXTRACTED]
- [[_sidebar.tsx]] - `imports_from` [EXTRACTED]
- [[extended-sidebar.tsx]] - `imports_from` [EXTRACTED]
- [[favorites-menu.tsx]] - `imports_from` [EXTRACTED]
- [[index.ts_265]] - `imports_from` [EXTRACTED]
- [[layout.tsx_4]] - `imports_from` [EXTRACTED]
- [[projects-list.tsx]] - `imports_from` [EXTRACTED]
- [[quick-actions.tsx_1]] - `imports_from` [EXTRACTED]
- [[resizable-sidebar.tsx]] - `imports_from` [EXTRACTED]
- [[sidebar-menu-items.tsx]] - `imports_from` [EXTRACTED]
- [[sidebar-wrapper.tsx]] - `imports_from` [EXTRACTED]
- [[use-app-theme.ts]] - `imports_from` [EXTRACTED]
- [[use-favorite.ts]] - `imports_from` [EXTRACTED]
- [[useAppTheme()]] - `imports` [EXTRACTED]
- [[useFavorite()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Sidebar_Resizable
