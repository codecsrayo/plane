---
source_file: "web/core/store/issue/root.store.ts"
type: "code"
community: "Calendar Issue"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Calendar_Issue
---

# root.store.ts

## Connections

- [[CalendarStore]] - `imports` [EXTRACTED]
- [[ICalendarStore]] - `imports` [EXTRACTED]
- [[IIssueKanBanViewStore]] - `imports` [EXTRACTED]
- [[IIssueRootStore]] - `contains` [EXTRACTED]
- [[IIssueStore]] - `imports` [EXTRACTED]
- [[IWorkspaceMembership_1]] - `imports` [EXTRACTED]
- [[IssueKanBanViewStore]] - `imports` [EXTRACTED]
- [[IssueRootStore]] - `contains` [EXTRACTED]
- [[IssueStore]] - `imports` [EXTRACTED]
- [[attachment.store.ts]] - `imports_from` [EXTRACTED]
- [[base-issues.store.ts]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_5]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_6]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_7]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_8]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_9]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_10]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_11]] - `imports_from` [EXTRACTED]
- [[filter.store.ts_12]] - `imports_from` [EXTRACTED]
- [[index.ts_273]] - `imports_from` [EXTRACTED]
- [[index.ts_280]] - `imports_from` [EXTRACTED]
- [[index.ts_276]] - `imports_from` [EXTRACTED]
- [[index.ts_278]] - `imports_from` [EXTRACTED]
- [[index.ts_279]] - `imports_from` [EXTRACTED]
- [[index.ts_274]] - `imports_from` [EXTRACTED]
- [[index.ts_275]] - `imports_from` [EXTRACTED]
- [[index.ts_277]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_5]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_6]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_7]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_8]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_9]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_10]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_11]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_12]] - `imports_from` [EXTRACTED]
- [[issue.store.ts_13]] - `imports_from` [EXTRACTED]
- [[issue_calendar_view.store.ts]] - `imports_from` [EXTRACTED]
- [[issue_kanban_view.store.ts]] - `imports_from` [EXTRACTED]
- [[root.store.ts_1]] - `imports_from` [EXTRACTED]
- [[root.store.ts_2]] - `imports_from` [EXTRACTED]
- [[root.store.ts_4]] - `imports_from` [EXTRACTED]
- [[workspace-member.store.ts]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Calendar_Issue
