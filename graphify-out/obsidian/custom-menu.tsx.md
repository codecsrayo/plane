---
source_file: "ui/src/dropdowns/custom-menu.tsx"
type: "code"
community: "Custom Menu"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Custom_Menu
---

# custom-menu.tsx

## Connections

- [[CustomMenu()]] - `contains` [EXTRACTED]
- [[ICustomMenuDropdownProps]] - `imports` [EXTRACTED]
- [[ICustomMenuItemProps]] - `imports` [EXTRACTED]
- [[ICustomSubMenuContentProps]] - `imports` [EXTRACTED]
- [[ICustomSubMenuProps]] - `imports` [EXTRACTED]
- [[ICustomSubMenuTriggerProps]] - `imports` [EXTRACTED]
- [[MenuContext]] - `contains` [EXTRACTED]
- [[MenuItem()]] - `contains` [EXTRACTED]
- [[Portal()]] - `contains` [EXTRACTED]
- [[PortalProps]] - `contains` [EXTRACTED]
- [[SubMenu()]] - `contains` [EXTRACTED]
- [[SubMenuContent()]] - `contains` [EXTRACTED]
- [[SubMenuContext]] - `contains` [EXTRACTED]
- [[SubMenuTrigger()]] - `contains` [EXTRACTED]
- [[block.tsx]] - `imports_from` [EXTRACTED]
- [[helper.tsx_3]] - `imports_from` [EXTRACTED]
- [[index.ts_14]] - `imports_from` [EXTRACTED]
- [[use-dropdown-key-down.tsx]] - `imports_from` [EXTRACTED]
- [[useDropdownKeyDown()]] - `imports` [EXTRACTED]
- [[useSubMenu()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Custom_Menu
