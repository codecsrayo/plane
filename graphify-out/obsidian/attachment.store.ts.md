---
source_file: "web/core/store/issue/issue-details/attachment.store.ts"
type: "code"
community: "Issue Store & Filters Logic"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Issue_Store__Filters_Logic
---

# attachment.store.ts

## Connections
- [[IIssueAttachmentStore]] - `contains` [EXTRACTED]
- [[IIssueAttachmentStoreActions]] - `contains` [EXTRACTED]
- [[IIssueDetail]] - `imports` [EXTRACTED]
- [[IIssueRootStore]] - `imports` [EXTRACTED]
- [[IssueAttachmentStore]] - `contains` [EXTRACTED]
- [[TAttachmentUploadStatus]] - `contains` [EXTRACTED]
- [[api.service.ts_1]] - `imports_from` [EXTRACTED]
- [[asset.store.ts]] - `imports_from` [EXTRACTED]
- [[attachment-list-upload-item.tsx]] - `imports_from` [EXTRACTED]
- [[attachment-upload-details.tsx]] - `imports_from` [EXTRACTED]
- [[attachments]] - `contains` [EXTRACTED]
- [[attachmentsUploadStatus]] - `contains` [EXTRACTED]
- [[helper.tsx_27]] - `imports_from` [EXTRACTED]
- [[index.ts_284]] - `imports_from` [EXTRACTED]
- [[newAttachmentIds]] - `contains` [EXTRACTED]
- [[progressPercentage_1]] - `contains` [EXTRACTED]
- [[root.store.ts_3]] - `imports_from` [EXTRACTED]
- [[root.store.ts_4]] - `imports_from` [EXTRACTED]
- [[tempId_1]] - `contains` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Issue_Store__Filters_Logic