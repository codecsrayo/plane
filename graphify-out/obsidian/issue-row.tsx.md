---
source_file: "web/core/components/issues/issue-layouts/spreadsheet/issue-row.tsx"
type: "code"
community: "Issue Activity"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Issue_Activity
---

# issue-row.tsx

## Connections

- [[IssueColumn]] - `imports` [EXTRACTED]
- [[IssueIdentifier]] - `imports` [EXTRACTED]
- [[IssueRowDetails]] - `contains` [EXTRACTED]
- [[IssueRowDetailsProps]] - `contains` [EXTRACTED]
- [[Props_269]] - `contains` [EXTRACTED]
- [[SpreadsheetIssueRow]] - `contains` [EXTRACTED]
- [[TSelectionHelper]] - `imports` [EXTRACTED]
- [[index.ts_443]] - `imports_from` [EXTRACTED]
- [[isIssueNew()]] - `imports` [EXTRACTED]
- [[issue-column.tsx]] - `imports_from` [EXTRACTED]
- [[render-if-visible-HOC.tsx]] - `imports_from` [EXTRACTED]
- [[spreadsheet-table.tsx]] - `imports_from` [EXTRACTED]
- [[use-issue-detail.ts]] - `imports_from` [EXTRACTED]
- [[use-issue-peek-overview-redirection.tsx]] - `imports_from` [EXTRACTED]
- [[use-issues.ts]] - `imports_from` [EXTRACTED]
- [[use-multiple-select.ts]] - `imports_from` [EXTRACTED]
- [[use-platform-os.tsx_1]] - `imports_from` [EXTRACTED]
- [[use-project.ts]] - `imports_from` [EXTRACTED]
- [[useIssueDetail()]] - `imports` [EXTRACTED]
- [[useIssues()]] - `imports` [EXTRACTED]
- [[usePlatformOS()]] - `imports` [EXTRACTED]
- [[useProject()]] - `imports` [EXTRACTED]
- [[utils.tsx_2]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Issue_Activity
