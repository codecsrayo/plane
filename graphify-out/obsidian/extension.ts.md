---
source_file: "editor/src/core/constants/extension.ts"
type: "code"
community: "Community 30"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_30
---

# extension.ts

## Connections
- [[BLOCK_NODE_TYPES]] - `contains` [EXTRACTED]
- [[CORE_EXTENSIONS]] - `contains` [EXTRACTED]
- [[asset.ts_1]] - `imports_from` [EXTRACTED]
- [[block-menu-options.tsx]] - `imports_from` [EXTRACTED]
- [[block-menu.tsx]] - `imports_from` [EXTRACTED]
- [[code-block.ts]] - `imports_from` [EXTRACTED]
- [[color-selector.tsx_2]] - `imports_from` [EXTRACTED]
- [[common.ts_1]] - `imports_from` [EXTRACTED]
- [[custom-color.ts]] - `imports_from` [EXTRACTED]
- [[delete-key-shortcut.ts]] - `imports_from` [EXTRACTED]
- [[drag-handle.ts]] - `imports_from` [EXTRACTED]
- [[drag-handle.tsx_1]] - `imports_from` [EXTRACTED]
- [[drag-handle.tsx_2]] - `imports_from` [EXTRACTED]
- [[editor-commands.ts]] - `imports_from` [EXTRACTED]
- [[editor-container.tsx]] - `imports_from` [EXTRACTED]
- [[editor-ref.ts]] - `imports_from` [EXTRACTED]
- [[enter-key.ts]] - `imports_from` [EXTRACTED]
- [[extension-config.ts]] - `imports_from` [EXTRACTED]
- [[extension-config.ts_1]] - `imports_from` [EXTRACTED]
- [[extension-config.ts_3]] - `imports_from` [EXTRACTED]
- [[extension.ts_2]] - `imports_from` [EXTRACTED]
- [[extension.tsx]] - `imports_from` [EXTRACTED]
- [[extension.tsx_2]] - `imports_from` [EXTRACTED]
- [[extension.tsx_4]] - `imports_from` [EXTRACTED]
- [[headings-list.ts]] - `imports_from` [EXTRACTED]
- [[helpers.ts]] - `imports_from` [EXTRACTED]
- [[horizontal-rule.ts]] - `imports_from` [EXTRACTED]
- [[image-helpers.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_53]] - `imports_from` [EXTRACTED]
- [[index.tsx_2]] - `imports_from` [EXTRACTED]
- [[insert-empty-paragraph-at-node-boundary.ts]] - `imports_from` [EXTRACTED]
- [[insert-line-above-table-action.ts]] - `imports_from` [EXTRACTED]
- [[insert-line-below-table-action.ts]] - `imports_from` [EXTRACTED]
- [[keymap.ts]] - `imports_from` [EXTRACTED]
- [[link-selector.tsx]] - `imports_from` [EXTRACTED]
- [[list-helpers.ts]] - `imports_from` [EXTRACTED]
- [[list-keymap.ts]] - `imports_from` [EXTRACTED]
- [[menu-items.ts]] - `imports_from` [EXTRACTED]
- [[placeholder.ts]] - `imports_from` [EXTRACTED]
- [[quote.ts]] - `imports_from` [EXTRACTED]
- [[root.tsx_3]] - `imports_from` [EXTRACTED]
- [[root.tsx_4]] - `imports_from` [EXTRACTED]
- [[side-menu.ts]] - `imports_from` [EXTRACTED]
- [[suggestion.ts]] - `imports_from` [EXTRACTED]
- [[table-cell.ts]] - `imports_from` [EXTRACTED]
- [[table-header.ts]] - `imports_from` [EXTRACTED]
- [[table-row.ts]] - `imports_from` [EXTRACTED]
- [[table.ts]] - `imports_from` [EXTRACTED]
- [[trailing-node.ts]] - `imports_from` [EXTRACTED]
- [[uploader.tsx]] - `imports_from` [EXTRACTED]
- [[utility.ts_1]] - `imports_from` [EXTRACTED]
- [[utils.ts_8]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_30