---
source_file: "store/issue/helpers/base-issues-utils.ts"
type: "code"
community: "Issue Group"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Issue_Group
---

# base-issues-utils.ts

## Connections

- [[EIssueGroupedAction]] - `imports` [EXTRACTED]
- [[ISSUE_GROUP_BY_KEY]] - `imports` [EXTRACTED]
- [[base-issues.store.ts]] - `imports_from` [EXTRACTED]
- [[checkIssueDateFilter()]] - `contains` [EXTRACTED]
- [[getDifference()]] - `contains` [EXTRACTED]
- [[getFilteredWorkItems()]] - `contains` [EXTRACTED]
- [[getGroupIssueKeyActions()]] - `contains` [EXTRACTED]
- [[getGroupKey()]] - `contains` [EXTRACTED]
- [[getGroupedWorkItemIds()]] - `contains` [EXTRACTED]
- [[getIssueIds()]] - `contains` [EXTRACTED]
- [[getOrderedWorkItems()]] - `contains` [EXTRACTED]
- [[getPreviousIssuesState()]] - `contains` [EXTRACTED]
- [[getSortOrderToFilterEmptyValues()]] - `contains` [EXTRACTED]
- [[getSubGroupIssueKeyActions()]] - `contains` [EXTRACTED]
- [[store]] - `imports` [EXTRACTED]
- [[store-context.tsx]] - `imports_from` [EXTRACTED]
- [[sub_issues_filter.store.ts]] - `imports_from` [EXTRACTED]
- [[updateSubWorkItemFilters()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Issue_Group
