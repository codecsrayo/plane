---
source_file: "web/core/store/issue/issue-details/sub_issues_filter.store.ts"
type: "code"
community: "Community 196"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_196
---

# sub_issues_filter.store.ts

## Connections

- [[DEFAULT_DISPLAY_PROPERTIES]] - `contains` [EXTRACTED]
- [[IWorkItemSubIssueFiltersStore]] - `contains` [EXTRACTED]
- [[IssueSubIssuesStore]] - `imports` [EXTRACTED]
- [[WorkItemSubIssueFiltersStore]] - `contains` [EXTRACTED]
- [[base-issues-utils.ts]] - `imports_from` [EXTRACTED]
- [[filteredWorkItems]] - `contains` [EXTRACTED]
- [[getFilteredWorkItems()]] - `imports` [EXTRACTED]
- [[getGroupedWorkItemIds()]] - `imports` [EXTRACTED]
- [[groupedWorkItemIds]] - `contains` [EXTRACTED]
- [[subIssueFilters]] - `contains` [EXTRACTED]
- [[subIssueIds]] - `contains` [EXTRACTED]
- [[sub_issues.store.ts]] - `imports_from` [EXTRACTED]
- [[updateSubWorkItemFilters()]] - `imports` [EXTRACTED]
- [[utils.tsx_1]] - `imports_from` [EXTRACTED]
- [[workItems]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_196
