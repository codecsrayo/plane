---
source_file: "web/core/components/power-k/ui/pages/context-based/work-item/root.tsx"
type: "code"
community: "Community 61"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_61
---

# root.tsx

## Connections

- [[PowerKMembersMenu]] - `imports` [EXTRACTED]
- [[PowerKProjectStatesMenu]] - `imports` [EXTRACTED]
- [[PowerKWorkItemContextBasedPages]] - `contains` [EXTRACTED]
- [[PowerKWorkItemCyclesMenu]] - `imports` [EXTRACTED]
- [[PowerKWorkItemEstimatesMenu]] - `imports` [EXTRACTED]
- [[PowerKWorkItemLabelsMenu]] - `imports` [EXTRACTED]
- [[PowerKWorkItemModulesMenu]] - `imports` [EXTRACTED]
- [[PowerKWorkItemPrioritiesMenu]] - `imports` [EXTRACTED]
- [[Props_321]] - `contains` [EXTRACTED]
- [[TPowerKPageType]] - `imports` [EXTRACTED]
- [[cycles-menu.tsx]] - `imports_from` [EXTRACTED]
- [[estimates-menu.tsx]] - `imports_from` [EXTRACTED]
- [[labels-menu.tsx]] - `imports_from` [EXTRACTED]
- [[members.tsx_7]] - `imports_from` [EXTRACTED]
- [[modules-menu.tsx]] - `imports_from` [EXTRACTED]
- [[priorities-menu.tsx]] - `imports_from` [EXTRACTED]
- [[states-menu.tsx]] - `imports_from` [EXTRACTED]
- [[types.ts_2]] - `imports_from` [EXTRACTED]
- [[use-issue-detail.ts]] - `imports_from` [EXTRACTED]
- [[use-member.ts]] - `imports_from` [EXTRACTED]
- [[useIssueDetail()]] - `imports` [EXTRACTED]
- [[useMember()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_61
