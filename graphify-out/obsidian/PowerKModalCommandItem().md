---
source_file: "web/core/components/power-k/ui/modal/command-item.tsx"
type: "code"
community: "Community 124"
location: "L28"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_124
---

# PowerKModalCommandItem()

## Connections
- [[builder.tsx]] - `imports` [EXTRACTED]
- [[command-item.tsx]] - `contains` [EXTRACTED]
- [[command.tsx_1]] - `imports` [EXTRACTED]
- [[estimates-menu.tsx]] - `imports` [EXTRACTED]
- [[languages-menu.tsx]] - `imports` [EXTRACTED]
- [[members.tsx_8]] - `imports` [EXTRACTED]
- [[no-results-command.tsx]] - `imports` [EXTRACTED]
- [[priorities-menu.tsx]] - `imports` [EXTRACTED]
- [[search-results.tsx]] - `imports` [EXTRACTED]
- [[start-of-week-menu.tsx]] - `imports` [EXTRACTED]
- [[state-menu-item.tsx]] - `imports` [EXTRACTED]
- [[status-menu.tsx]] - `imports` [EXTRACTED]
- [[themes-menu.tsx]] - `imports` [EXTRACTED]
- [[timezone-menu.tsx]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_124