---
source_file: "web/core/components/inbox/content/issue-root.tsx"
type: "code"
community: "Activity & Inbox Filters"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Activity__Inbox_Filters
---

# issue-root.tsx

## Connections
- [[DescriptionInputLoader()]] - `imports` [EXTRACTED]
- [[IInboxIssueStore]] - `imports` [EXTRACTED]
- [[InboxIssueContentProperties]] - `imports` [EXTRACTED]
- [[InboxIssueMainContent]] - `contains` [EXTRACTED]
- [[IssueTitleInput]] - `imports` [EXTRACTED]
- [[Props_451]] - `contains` [EXTRACTED]
- [[inbox-issue.store.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_265]] - `imports_from` [EXTRACTED]
- [[index.ts_288]] - `imports_from` [EXTRACTED]
- [[index.ts_319]] - `imports_from` [EXTRACTED]
- [[index.ts_367]] - `imports_from` [EXTRACTED]
- [[index.ts_368]] - `imports_from` [EXTRACTED]
- [[index.ts_370]] - `imports_from` [EXTRACTED]
- [[index.ts_376]] - `imports_from` [EXTRACTED]
- [[index.ts_441]] - `imports_from` [EXTRACTED]
- [[intakeWorkItemVersionService]] - `contains` [EXTRACTED]
- [[issue-properties.tsx]] - `imports_from` [EXTRACTED]
- [[loader.tsx_5]] - `imports_from` [EXTRACTED]
- [[root.tsx_115]] - `imports_from` [EXTRACTED]
- [[title-input.tsx]] - `imports_from` [EXTRACTED]
- [[use-issue-detail.ts]] - `imports_from` [EXTRACTED]
- [[use-member.ts]] - `imports_from` [EXTRACTED]
- [[use-project-inbox.ts]] - `imports_from` [EXTRACTED]
- [[use-project.ts]] - `imports_from` [EXTRACTED]
- [[use-reload-confirmation.tsx]] - `imports_from` [EXTRACTED]
- [[useDebouncedDuplicateIssues()]] - `imports` [EXTRACTED]
- [[useIssueDetail()]] - `imports` [EXTRACTED]
- [[useMember()]] - `imports` [EXTRACTED]
- [[useProject()]] - `imports` [EXTRACTED]
- [[useProjectInbox()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Activity__Inbox_Filters