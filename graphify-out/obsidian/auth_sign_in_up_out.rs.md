---
source_file: "api_rust/tests/auth_sign_in_up_out.rs"
type: "code"
community: "Returns Sign"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Returns_Sign
---

# auth_sign_in_up_out.rs

## Connections

- [[location_has_error()]] - `contains` [EXTRACTED]
- [[sign_in_invalid_email_redirects_with_error()]] - `contains` [EXTRACTED]
- [[sign_in_missing_credentials_redirects_with_error()]] - `contains` [EXTRACTED]
- [[sign_in_unknown_user_redirects_with_error()]] - `contains` [EXTRACTED]
- [[sign_in_valid_credentials_redirects_to_success()]] - `contains` [EXTRACTED]
- [[sign_in_wrong_password_redirects_with_error()]] - `contains` [EXTRACTED]
- [[sign_out_without_session_returns_401()]] - `contains` [EXTRACTED]
- [[sign_up_duplicate_email_redirects_with_error()]] - `contains` [EXTRACTED]
- [[sign_up_rejects_invalid_emails()]] - `contains` [EXTRACTED]
- [[sign_up_with_valid_credentials_redirects_to_success()]] - `contains` [EXTRACTED]
- [[sign_up_without_email_redirects_with_error()]] - `contains` [EXTRACTED]
- [[sign_up_without_password_redirects_with_error()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Returns_Sign
