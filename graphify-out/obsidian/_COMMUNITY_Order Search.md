---
type: community
cohesion: 0.15
members: 16
---

# Order Search

**Cohesion:** 0.15 - loosely connected
**Members:** 16 nodes

## Members

- [[PAGE_SORTING_KEY_OPTIONS]] - code - web/core/components/pages/list/order-by.tsx
- [[PageOrderByDropdown()]] - code - web/core/components/pages/list/order-by.tsx
- [[PageSearchInput()]] - code - web/core/components/pages/list/search-input.tsx
- [[PageTabNavigation()]] - code - web/core/components/pages/list/tab-navigation.tsx
- [[PagesListHeaderRoot]] - code - web/core/components/pages/header/root.tsx
- [[Props_467]] - code - web/core/components/pages/header/root.tsx
- [[Props_471]] - code - web/core/components/pages/list/order-by.tsx
- [[Props_469]] - code - web/core/components/pages/list/search-input.tsx
- [[TPageTabNavigation]] - code - web/core/components/pages/list/tab-navigation.tsx
- [[index.ts_421]] - code - web/core/components/pages/list/applied-filters/index.ts
- [[index.ts_420]] - code - web/core/components/pages/list/filters/index.ts
- [[order-by.tsx_5]] - code - web/core/components/pages/list/order-by.tsx
- [[pageTabs]] - code - web/core/components/pages/list/tab-navigation.tsx
- [[root.tsx_124]] - code - web/core/components/pages/header/root.tsx
- [[search-input.tsx]] - code - web/core/components/pages/list/search-input.tsx
- [[tab-navigation.tsx]] - code - web/core/components/pages/list/tab-navigation.tsx

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/Order_Search
SORT file.name ASC
```

## Connections to other communities

- 3 edges to [[_COMMUNITY_Member Members]]
- 1 edge to [[_COMMUNITY_Control Move]]

## Top bridge nodes

- [[root.tsx_124]] - degree 13, connects to 1 community
- [[Props_467]] - degree 2, connects to 1 community
