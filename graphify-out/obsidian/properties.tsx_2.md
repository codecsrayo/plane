---
source_file: "web/core/components/issues/peek-overview/properties.tsx"
type: "code"
community: "Work Item Detail & Layouts"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Work_Item_Detail__Layouts
---

# properties.tsx

## Connections
- [[ButtonAvatars]] - `imports` [EXTRACTED]
- [[DateAlert()]] - `imports` [EXTRACTED]
- [[DateDropdown]] - `imports` [EXTRACTED]
- [[EstimateDropdown]] - `imports` [EXTRACTED]
- [[IPeekOverviewProperties]] - `contains` [EXTRACTED]
- [[IssueCycleSelect]] - `imports` [EXTRACTED]
- [[IssueModuleSelect]] - `imports` [EXTRACTED]
- [[IssueParentSelectRoot]] - `imports` [EXTRACTED]
- [[MemberDropdown]] - `imports` [EXTRACTED]
- [[PeekOverviewProperties]] - `contains` [EXTRACTED]
- [[PriorityDropdown()]] - `imports` [EXTRACTED]
- [[SidebarPropertyListItem()]] - `imports` [EXTRACTED]
- [[StateDropdown]] - `imports` [EXTRACTED]
- [[TransferHopInfo()]] - `imports` [EXTRACTED]
- [[WorkItemAdditionalSidebarProperties()]] - `imports` [EXTRACTED]
- [[avatar.tsx_2]] - `imports_from` [EXTRACTED]
- [[cycle-select.tsx]] - `imports_from` [EXTRACTED]
- [[date.tsx_1]] - `imports_from` [EXTRACTED]
- [[dropdown.tsx_5]] - `imports_from` [EXTRACTED]
- [[dropdown.tsx_6]] - `imports_from` [EXTRACTED]
- [[estimate.tsx_1]] - `imports_from` [EXTRACTED]
- [[index.ts_367]] - `imports_from` [EXTRACTED]
- [[index.ts_371]] - `imports_from` [EXTRACTED]
- [[main-content.tsx_1]] - `imports_from` [EXTRACTED]
- [[module-select.tsx]] - `imports_from` [EXTRACTED]
- [[priority.tsx_1]] - `imports_from` [EXTRACTED]
- [[property-list-item.tsx]] - `imports_from` [EXTRACTED]
- [[use-issue-detail.ts]] - `imports_from` [EXTRACTED]
- [[use-member.ts]] - `imports_from` [EXTRACTED]
- [[use-project-state.ts]] - `imports_from` [EXTRACTED]
- [[use-project.ts]] - `imports_from` [EXTRACTED]
- [[useIssueDetail()]] - `imports` [EXTRACTED]
- [[useMember()]] - `imports` [EXTRACTED]
- [[useProject()]] - `imports` [EXTRACTED]
- [[useProjectState()]] - `imports` [EXTRACTED]
- [[view.tsx_2]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Work_Item_Detail__Layouts