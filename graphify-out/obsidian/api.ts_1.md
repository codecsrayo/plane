---
source_file: "packages/e2e-utils/src/helpers/api.ts"
type: "code"
community: "Community 174"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_174
---

# api.ts

## Connections
- [[createProject()]] - `contains` [EXTRACTED]
- [[createWorkspace()]] - `contains` [EXTRACTED]
- [[ensureInstanceConfigured()]] - `contains` [EXTRACTED]
- [[extractErrorCode()]] - `contains` [EXTRACTED]
- [[fetchCsrfToken()]] - `contains` [EXTRACTED]
- [[getMe()]] - `contains` [EXTRACTED]
- [[getStates()]] - `contains` [EXTRACTED]
- [[global-setup.ts_1]] - `imports_from` [EXTRACTED]
- [[global-teardown.ts_1]] - `imports_from` [EXTRACTED]
- [[handleAuthRedirect()]] - `contains` [EXTRACTED]
- [[index.ts_552]] - `imports_from` [EXTRACTED]
- [[signIn()]] - `contains` [EXTRACTED]
- [[signUp()]] - `contains` [EXTRACTED]
- [[waitForHealth()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_174