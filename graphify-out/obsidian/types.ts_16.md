---
source_file: "web/core/components/power-k/core/types.ts"
type: "code"
community: "Power Context"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Power_Context
---

# types.ts

## Connections

- [[TCommandPaletteState]] - `contains` [EXTRACTED]
- [[TPowerKCommandConfig]] - `contains` [EXTRACTED]
- [[TPowerKCommandGroup]] - `contains` [EXTRACTED]
- [[TPowerKContext]] - `contains` [EXTRACTED]
- [[TPowerKContextType]] - `contains` [EXTRACTED]
- [[TPowerKContextTypeExtended]] - `imports` [EXTRACTED]
- [[TPowerKPageType]] - `contains` [EXTRACTED]
- [[TPowerKPageTypeExtended]] - `imports` [EXTRACTED]
- [[TPowerKSearchResultsKeys]] - `contains` [EXTRACTED]
- [[TPowerKSearchResultsKeysExtended]] - `imports` [EXTRACTED]
- [[TSelectionPageProps]] - `contains` [EXTRACTED]
- [[account-commands.ts]] - `imports_from` [EXTRACTED]
- [[base-power-k.store.ts]] - `imports_from` [EXTRACTED]
- [[command.ts]] - `imports_from` [EXTRACTED]
- [[command.tsx_1]] - `imports_from` [EXTRACTED]
- [[commands-list.tsx]] - `imports_from` [EXTRACTED]
- [[commands.ts]] - `imports_from` [EXTRACTED]
- [[commands.ts_1]] - `imports_from` [EXTRACTED]
- [[commands.ts_2]] - `imports_from` [EXTRACTED]
- [[commands.ts_3]] - `imports_from` [EXTRACTED]
- [[commands.ts_4]] - `imports_from` [EXTRACTED]
- [[commands.tsx]] - `imports_from` [EXTRACTED]
- [[constants.ts_6]] - `imports_from` [EXTRACTED]
- [[context-detector.ts_1]] - `imports_from` [EXTRACTED]
- [[context-indicator.tsx]] - `imports_from` [EXTRACTED]
- [[default.tsx_3]] - `imports_from` [EXTRACTED]
- [[global-shortcuts.tsx]] - `imports_from` [EXTRACTED]
- [[header.tsx_53]] - `imports_from` [EXTRACTED]
- [[help-commands.ts]] - `imports_from` [EXTRACTED]
- [[index.ts_399]] - `imports_from` [EXTRACTED]
- [[miscellaneous-commands.ts]] - `imports_from` [EXTRACTED]
- [[navigation.ts_1]] - `imports_from` [EXTRACTED]
- [[no-results-command.tsx]] - `imports_from` [EXTRACTED]
- [[preferences-commands.ts]] - `imports_from` [EXTRACTED]
- [[project-cycles-menu.tsx]] - `imports_from` [EXTRACTED]
- [[project-modules-menu.tsx]] - `imports_from` [EXTRACTED]
- [[project-settings-menu.tsx]] - `imports_from` [EXTRACTED]
- [[project-views-menu.tsx]] - `imports_from` [EXTRACTED]
- [[projects-app-provider.tsx]] - `imports_from` [EXTRACTED]
- [[registry.ts_1]] - `imports_from` [EXTRACTED]
- [[root.ts_2]] - `imports_from` [EXTRACTED]
- [[root.ts_3]] - `imports_from` [EXTRACTED]
- [[root.tsx_38]] - `imports_from` [EXTRACTED]
- [[root.tsx_98]] - `imports_from` [EXTRACTED]
- [[root.tsx_99]] - `imports_from` [EXTRACTED]
- [[root.tsx_100]] - `imports_from` [EXTRACTED]
- [[root.tsx_101]] - `imports_from` [EXTRACTED]
- [[root.tsx_102]] - `imports_from` [EXTRACTED]
- [[search-menu.tsx]] - `imports_from` [EXTRACTED]
- [[search-results-map.tsx_1]] - `imports_from` [EXTRACTED]
- [[shared.ts_11]] - `imports_from` [EXTRACTED]
- [[shared.ts_12]] - `imports_from` [EXTRACTED]
- [[shortcut-handler.ts]] - `imports_from` [EXTRACTED]
- [[shortcut.tsx]] - `imports_from` [EXTRACTED]
- [[top-nav-power-k.tsx]] - `imports_from` [EXTRACTED]
- [[use-context-indicator.ts]] - `imports_from` [EXTRACTED]
- [[use-extended-context-indicator.ts]] - `imports_from` [EXTRACTED]
- [[workspace-settings-menu.tsx]] - `imports_from` [EXTRACTED]
- [[wrapper.tsx_1]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Power_Context
