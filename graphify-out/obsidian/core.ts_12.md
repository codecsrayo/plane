---
source_file: "utils/src/rich-filters/operations/transformation/core.ts"
type: "code"
community: "Community 148"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_148
---

# core.ts

## Connections
- [[TTreeTransformFn]] - `contains` [EXTRACTED]
- [[TTreeTransformResult]] - `contains` [EXTRACTED]
- [[comparison.ts]] - `imports_from` [EXTRACTED]
- [[core.ts_14]] - `imports_from` [EXTRACTED]
- [[core.ts_15]] - `imports_from` [EXTRACTED]
- [[core.ts_13]] - `imports_from` [EXTRACTED]
- [[createGroupTransformResult()]] - `contains` [EXTRACTED]
- [[getGroupChildren()]] - `imports` [EXTRACTED]
- [[hasValidValue()]] - `imports` [EXTRACTED]
- [[isConditionNode()]] - `imports` [EXTRACTED]
- [[isGroupNode()]] - `imports` [EXTRACTED]
- [[removeNodeFromExpression()]] - `contains` [EXTRACTED]
- [[sanitizeAndStabilizeExpression()]] - `contains` [EXTRACTED]
- [[shared.ts_5]] - `imports_from` [EXTRACTED]
- [[shared.ts_6]] - `imports_from` [EXTRACTED]
- [[transformExpressionTree()]] - `contains` [EXTRACTED]
- [[transformGroup()]] - `imports` [EXTRACTED]
- [[transformGroupWithChildren()]] - `contains` [EXTRACTED]
- [[unwrapGroupIfNeeded()]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_148