---
source_file: "web/core/store/issue/workspace-draft/filter.store.ts"
type: "code"
community: "Community 149"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_149
---

# filter.store.ts

## Connections

- [[IBaseIssueFilterStore]] - `imports` [EXTRACTED]
- [[IIssueRootStore]] - `imports` [EXTRACTED]
- [[IWorkspaceDraftIssuesFilter]] - `contains` [EXTRACTED]
- [[IssueFilterHelperStore]] - `imports` [EXTRACTED]
- [[IssueFiltersService]] - `imports` [EXTRACTED]
- [[WorkspaceDraftIssuesFilter]] - `contains` [EXTRACTED]
- [[_filters_4]] - `contains` [EXTRACTED]
- [[api.service.ts]] - `imports_from` [EXTRACTED]
- [[displayFilters_6]] - `contains` [EXTRACTED]
- [[displayProperties_3]] - `contains` [EXTRACTED]
- [[filterParams_5]] - `contains` [EXTRACTED]
- [[issue-filter-helper.store.ts]] - `imports_from` [EXTRACTED]
- [[issue_filter.service.ts]] - `imports_from` [EXTRACTED]
- [[kanbanFilters_4]] - `contains` [EXTRACTED]
- [[paginationParams_4]] - `contains` [EXTRACTED]
- [[root.store.ts_1]] - `imports_from` [EXTRACTED]
- [[toApiError()]] - `imports` [EXTRACTED]
- [[updatedDisplayFilters_4]] - `contains` [EXTRACTED]
- [[updatedDisplayProperties_4]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_149
