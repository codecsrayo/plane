---
source_file: "web/core/constants/plans.tsx"
type: "code"
community: "Community 116"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Community_116
---

# plans.tsx

## Connections
- [[ComingSoonBadge()]] - `contains` [EXTRACTED]
- [[ForumIcon()]] - `contains` [EXTRACTED]
- [[PLANE_PLANS]] - `contains` [EXTRACTED]
- [[PLANS_COMPARISON_LIST]] - `contains` [EXTRACTED]
- [[PLANS_LIST]] - `contains` [EXTRACTED]
- [[PlanePlans]] - `contains` [EXTRACTED]
- [[TPlanDetail]] - `contains` [EXTRACTED]
- [[TPlanFeatureData]] - `contains` [EXTRACTED]
- [[TPlanFeatureDetails]] - `contains` [EXTRACTED]
- [[TPlanePlans]] - `contains` [EXTRACTED]
- [[TPlansComparisonDetails]] - `contains` [EXTRACTED]
- [[base.tsx_1]] - `imports_from` [EXTRACTED]
- [[feature-detail.tsx]] - `imports_from` [EXTRACTED]
- [[plan-detail.tsx]] - `imports_from` [EXTRACTED]
- [[root.tsx_22]] - `imports_from` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Community_116